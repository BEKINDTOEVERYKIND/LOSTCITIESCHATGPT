# Locked policy-frequency search cost v10

Version 9 passed preflight, every discovery/allocation barrier, and all 54
batched TRAIN jobs containing the complete 216 logical slices. Calibration
then failed before fitting because the native evaluator's burned-seed literal
omitted the retired v8 namespace while the frozen Python consumer required it.
An independent audit verified every GitHub artifact digest, every inner slice
sidecar, the complete ordered 0..13823 allocation range, exact completion, and
zero caps. This was a producer/consumer binding defect, not an AI result.

All 21 v9 roots are retired. Version 10 changes only that proven defect: the
native and Python burned-seed declarations are identical and a direct
cross-language source contract test prevents recurrence. All 216 TRAIN and all
192 SELECT/TEST slice boundaries, rows, budgets, filenames, reducers, and
adequacy requirements remain unchanged. It uses fresh `202709` roots and
changes no controller math, actor settings, sample size, model-adequacy
requirement, TEST rule, safety threshold, or final promotion gate. The terminal
record is `data/experiments/policy_cost_v9_run_33278577188_failure.json`.
...
seed `202709140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202709100101`.
- SELECT: 32,768 matches, seed `202709100201`.
- TEST: 32,768 matches, seed `202709100301`.

All later roots use the same fresh v10 split suffixes: primary
`202709110101/0201/0301`, fresh confirmation
`202709120101/0201/0301`, truth `202709130101/0201/0301`, and maintained
counterfactual `202709160101/0201/0301`. Calibration uses `202709140101`,
SELECT bootstrap uses `202709150101`, safety uses
`202709210101/0102`, and final uses `202709220101/0102`.
The retired namespaces include all v1--v9 production and smoke roots,
...
and every `20270829` feasibility-smoke seed. The v10 definition
smoke `202709010101` and every `20270929` smoke seed are likewise burned and
...
plus both canonical v10 exclusion manifests (text and JSON), are bound in the
...
`202709140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v10.py train-input` into
...
max-t bootstrap with seed `202709150101`. The 1% floor must beat the 2% floor
...
`tools/policy_cost_campaign_v10.py prepare-execution`. The workflow
