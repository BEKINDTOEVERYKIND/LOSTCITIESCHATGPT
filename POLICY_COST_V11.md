# Locked policy-frequency search cost v11

Version 10 passed its unique direct-addendum guard, materialized the frozen
parent, compiled the native runtime, and passed every substantive test reached.
Its preflight then ran an archived v9 definition-lifecycle test against the
current tree. That historical test correctly required the v9 execution file to
be absent at the old definition commit, but v9 had since launched, so the stale
current-tree assertion failed before transport upload, discovery, search, truth
labels, or any fixed-root seed use. This was a lifecycle-test isolation defect,
not an AI result.

Version 10 is never retried and all 21 of its unused roots are abandoned.
Version 11 changes only the proven defect: broad current-tree regression
discovery excludes every archived policy recovery module, while immutable
version-specific replays remain bound and the v11 recovery contract is run
explicitly. All 216 TRAIN and all 192 SELECT/TEST logical slice boundaries,
rows, budgets, filenames, reducers, adequacy requirements, and promotion gates
remain unchanged. It uses fresh `202710` roots and changes no controller math,
...
seed `202710140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202710100101`.
- SELECT: 32,768 matches, seed `202710100201`.
- TEST: 32,768 matches, seed `202710100301`.

All later roots use the same fresh v11 split suffixes: primary
`202710110101/0201/0301`, fresh confirmation
`202710120101/0201/0301`, truth `202710130101/0201/0301`, and maintained
counterfactual `202710160101/0201/0301`. Calibration uses `202710140101`,
SELECT bootstrap uses `202710150101`, safety uses
`202710210101/0102`, and final uses `202710220101/0102`.
The retired or abandoned namespaces include all v1--v10 production and smoke roots,
...
and every `20270929` feasibility-smoke seed. The v11 definition
smoke `202710010101` and every `20271029` smoke seed are likewise burned and
...
plus both canonical v11 exclusion manifests (text and JSON), are bound in the
...
`202710140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v11.py train-input` into
...
max-t bootstrap with seed `202710150101`. The 1% floor must beat the 2% floor
...
`tools/policy_cost_campaign_v11.py prepare-execution`. The workflow
