# Locked policy-frequency search cost v12

Version 11 passed preflight, every discovery and allocation contract, the
reservoir barrier, and all 54 TRAIN runner batches containing 216 logical
slices. Before calibration fitting, its frozen consumer rejected the one row
whose normalized semantic action's authoritative binary32 complete-move
probabilities sum to `1.0000000298023224` in double. This was a bounded
representation/serialization defect, not an AI result; SELECT, untouched TEST,
safety, final evidence, and promotion were never reached.

Version 11 is never retried and all 21 roots are retired. Version 12 changes
only the proven defect: semantic-action sums above one are canonicalized to one
only within eight binary32 epsilons, conditional-draw ratios preserve the exact
raw denominator, and every material overshoot still fails closed. All 216 TRAIN
and all 192 SELECT/TEST logical slice boundaries,
rows, budgets, filenames, reducers, adequacy requirements, and promotion gates
remain unchanged. It uses fresh `202711` roots and changes no controller math,
actor setting, sample size, TEST rule, safety threshold, or final gate. The
terminal record is
...
The candidate actor family remains `rolloutu5`; all earlier actor families
retain their byte-identical parser semantics. Every deployed artifact freezes source
seed `202711140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202711100101`.
- SELECT: 32,768 matches, seed `202711100201`.
- TEST: 32,768 matches, seed `202711100301`.

All later roots use the same fresh v12 split suffixes: primary
`202711110101/0201/0301`, fresh confirmation
`202711120101/0201/0301`, truth `202711130101/0201/0301`, and maintained
counterfactual `202711160101/0201/0301`. Calibration uses `202711140101`,
SELECT bootstrap uses `202711150101`, safety uses
`202711210101/0102`, and final uses `202711220101/0102`.
The retired or abandoned namespaces include all v1--v11 production and smoke roots,
including every `20270310/11/12/13/14/15/16/21/22` root, definition smoke
`202703010101`, every `20270329` feasibility-smoke seed, every v6
`20270410/11/12/13/14/15/16/21/22` root, definition smoke `202704010101`,
every `20270429` feasibility-smoke seed, every v7
`20270510/11/12/13/14/15/16/21/22` root, definition smoke `202705010101`,
every `20270529` feasibility-smoke seed, every v8
`20270710/11/12/13/14/15/16/21/22` root, definition smoke `202707010101`,
every `20270729` feasibility-smoke seed, every v9
`20270810/11/12/13/14/15/16/21/22` root, definition smoke `202708010101`,
every `20270829` feasibility-smoke seed, every v10
`20270910/11/12/13/14/15/16/21/22` root, definition smoke `202709010101`,
every `20270929` feasibility-smoke seed, every v11
`20271010/11/12/13/14/15/16/21/22` root, definition smoke `202710010101`,
and every `20271029` feasibility-smoke seed. The v12 definition
smoke `202711010101` and every `20271129` smoke seed are likewise burned and
cannot enter production evidence.
...
Generation and allocation are now separate workflow jobs for every split.
Immediately after the monolithic collector exits successfully, the generation
job hashes and uploads the raw discovery plus reservoir as one immutable
split-specific artifact. The allocation job downloads those exact bytes,
rechecks both SHA-256 digests, validates the census, and only then writes an
allocation. Allocation failure can therefore retain the already generated
policy-only source evidence without permitting a top-up, retry, or seed reuse.
Each producer and allocator writes a canonical run/attempt/head-bound
disposition and log. Its artifact upload uses the unconditional retention
path, and only a later enforcement step may fail the job. Infrastructure
retention independently downloads and classifies all three generation
artifacts and all three allocation-attempt artifacts, so a rejected allocation
cannot be mistaken for absent or incomplete native generation.

The discovery census has exactly five `union_width_counts` entries, one for
each possible master width 1 through 5. The native `PC_UNION_MAX`, the frozen
JSON schema, and the allocator's `UNION_MAX` are all five. Four- or six-entry
histograms, disagreement with the 1%-mask histogram, or any count-sum mismatch
fail closed before search or truth labels. The separate truth-support bound of
six is not a union-width histogram dimension.
...
search, or truth. The four canonical exact-17 audit artifacts and their hashes,
plus both canonical v12 exclusion manifests (text and JSON), are bound in the
plan and execution. The states, alternatives, evaluations,
...
The canonical Objective-3 v2 prerequisite is `final_passed`, so the target is
full-match hybrid value. That disposition and target are still rederived and
verified mechanically before discovery; any mismatch fails closed rather than
falling back to current-round margin.
...
the fixed poststrata are `J=0`, `J=1`, and `J>=2`, with respective quotas
`22`, `21`, and `21`. The tail group is a sampling group only: the exact slot
remains bound in every record, and every selected record evaluates its complete
vector. This preserves transitive comparisons among 1%, 4%, and higher-policy
alternatives without forcing rare random slots to meet arbitrary standalone
quotas.
...
`202711140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
`lc-policy-cost-v2-calibration-gate-v1` seals the unchanged calibration payload
...
The TRAIN TSV is converted once by `policy_cost_campaign_v12.py train-input` into
...
max-t bootstrap with seed `202711150101`. The 1% floor must beat the 2% floor
...
real addendum can be generated only after the terminal v8 failure record and
Objective-3 prerequisite are persisted, inside a clean `git archive` of the
exact definition parent, by
`tools/policy_cost_campaign_v12.py prepare-execution`. The workflow
