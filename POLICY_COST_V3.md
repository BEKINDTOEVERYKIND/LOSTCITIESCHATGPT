# Locked policy-frequency search cost v3

Version 2 stopped in preflight on GitHub run `33095561493`.  The locked native
self-tests and all 17 information-view suit-orbit hashes agreed, but the
checked-in exact-17 JSON had incorrectly made a developer-host ELF digest part
of its canonical portable identity.  No discovery, search, truth, arena, or
efficacy job ran.  Every v2 root is retired.  Version 3 binds the dataset
source that defines the exclusion semantics; preflight still seals the actual
runtime ELF dynamically in the build identity and source-free transport.
...
seed `202701140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202701100101`.
- SELECT: 32,768 matches, seed `202701100201`.
- TEST: 32,768 matches, seed `202701100301`.
...
`202701140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v3.py train-input` into
...
max-t bootstrap with seed `202701150101`. The 1% floor must beat the 2% floor
...
`tools/policy_cost_campaign_v3.py prepare-execution`. The workflow
