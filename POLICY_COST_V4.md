# Locked policy-frequency search cost v4

Version 3 passed its complete preflight on GitHub run `33108082105`, then all
three locked discovery jobs generated their complete atomic raw discovery and
reservoir files. The native collector correctly emitted five
`union_width_counts` entries for `PC_UNION_MAX=5`; the Python allocator had an
incorrect hard-coded requirement for six, so TRAIN, SELECT, and TEST each
failed closed with `invalid discovery union_width_counts`. The workflow placed
the artifact upload after allocation, so the failed runners discarded those
six generated files. Only the source-free transport and terminal
infrastructure bundle remain. No search label, truth label, calibration,
selection, TEST, arena, or efficacy reducer ran.

Only the three v3 discovery roots were physically used, but the one-shot
contract retires all 21 v3 roots. Version 4 makes the producer and consumer
share the exact five-entry histogram contract and separates generation plus
upload from allocation. It uses fresh `202702` roots and changes no controller
math, actor settings, sample size, model-adequacy requirement, TEST rule,
safety threshold, or final promotion gate. The canonical terminal record is
...
seed `202702140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202702100101`.
- SELECT: 32,768 matches, seed `202702100201`.
- TEST: 32,768 matches, seed `202702100301`.

All later roots use the same fresh v4 split suffixes: primary
`202702110101/0201/0301`, fresh confirmation
`202702120101/0201/0301`, truth `202702130101/0201/0301`, and maintained
counterfactual `202702160101/0201/0301`. Calibration uses `202702140101`,
SELECT bootstrap uses `202702150101`, safety uses
`202702210101/0102`, and final uses `202702220101/0102`.
The retired v3 namespaces explicitly include definition smoke
`202701010101`, every fixed root in families
`20270110/11/12/13/14/15/16/21/22`, and every `20270129` feasibility-smoke
seed. The v4 definition smoke `202702010101` and every `20270229` smoke seed
are likewise burned and cannot enter production evidence.
...
plus both canonical v4 exclusion manifests (text and JSON), are bound in the
...
`202702140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v4.py train-input` into
...
max-t bootstrap with seed `202702150101`. The 1% floor must beat the 2% floor
...
`tools/policy_cost_campaign_v4.py prepare-execution`. The workflow
