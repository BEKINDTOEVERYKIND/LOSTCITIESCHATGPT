# Locked policy-frequency search cost v5

Version 4 passed preflight and completed all three locked, cap-free discovery
jobs on GitHub run `33158323330`. TRAIN allocation passed, while SELECT and
untouched TEST allocation failed closed before any search or truth label. The
old allocator treated the outcome-blind hash slot `J` as five mandatory
poststrata and assigned a positive data-dependent quota to every nonempty
slot. Rare `J=3` and `J=4` cells had only 1--11 states, below their 12--16 row
quotas. This was a sampling-contract defect, not AI efficacy evidence.

All 21 v4 roots are retired. Version 5 changes only that proven allocation
defect: each base uses native poststrata `J=0`, `J=1`, and `J>=2` with fixed
quotas `22/21/21`. Every record retains its exact hashed slot and still
evaluates the complete max-five vector. Retained v4 censuses show every new
poststratum can satisfy its quota and eight-source minimum. V5 uses fresh
`202703` roots and changes no controller math, actor settings, sample size,
model-adequacy requirement, TEST rule, safety threshold, or final promotion
...
seed `202703140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202703100101`.
- SELECT: 32,768 matches, seed `202703100201`.
- TEST: 32,768 matches, seed `202703100301`.

All later roots use the same fresh v5 split suffixes: primary
`202703110101/0201/0301`, fresh confirmation
`202703120101/0201/0301`, truth `202703130101/0201/0301`, and maintained
counterfactual `202703160101/0201/0301`. Calibration uses `202703140101`,
SELECT bootstrap uses `202703150101`, safety uses
`202703210101/0102`, and final uses `202703220101/0102`.
The retired namespaces include all v1--v4 production and smoke roots,
including every `20270210/11/12/13/14/15/16/21/22` root, definition smoke
`202702010101`, and every `20270229` feasibility-smoke seed. The v5 definition
smoke `202703010101` and every `20270329` smoke seed are likewise burned and
...
plus both canonical v5 exclusion manifests (text and JSON), are bound in the
...
`202703140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v5.py train-input` into
...
max-t bootstrap with seed `202703150101`. The 1% floor must beat the 2% floor
...
real addendum can be generated only after the terminal v3 failure record and
Objective-3 prerequisite are persisted, inside a clean `git archive` of the
exact definition parent, by
`tools/policy_cost_campaign_v5.py prepare-execution`. The workflow
