# Locked policy-frequency search cost v6

Version 5 passed preflight, all discovery/allocation barriers, and all 216
locked TRAIN-evaluation shards on GitHub run `33176041169`. Calibration then
failed before producing a decision because one constant 512-world terminal
sample serialized its standard error as the invalid JSON token `-nan`.
Independent download reproduced the artifact and SHA-256 exactly: the old
`sum_squares - n*mean^2` calculation rounded seven ulps below zero. This was a
deterministic numerical serialization defect, not AI efficacy evidence.

All 21 v5 roots are retired. Version 6 changes only that proven defect: sample
variance is accumulated in a second pass from squared residuals, which is
non-negative by construction, and the native self-test reproduces the exact
constant-value failure. It uses fresh `202704` roots and changes no controller
...
seed `202704140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202704100101`.
- SELECT: 32,768 matches, seed `202704100201`.
- TEST: 32,768 matches, seed `202704100301`.

All later roots use the same fresh v6 split suffixes: primary
`202704110101/0201/0301`, fresh confirmation
`202704120101/0201/0301`, truth `202704130101/0201/0301`, and maintained
counterfactual `202704160101/0201/0301`. Calibration uses `202704140101`,
SELECT bootstrap uses `202704150101`, safety uses
`202704210101/0102`, and final uses `202704220101/0102`.
The retired namespaces include all v1--v5 production and smoke roots,
including every `20270310/11/12/13/14/15/16/21/22` root, definition smoke
`202703010101`, and every `20270329` feasibility-smoke seed. The v6 definition
smoke `202704010101` and every `20270429` smoke seed are likewise burned and
...
plus both canonical v6 exclusion manifests (text and JSON), are bound in the
...
`202704140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v6.py train-input` into
...
max-t bootstrap with seed `202704150101`. The 1% floor must beat the 2% floor
...
real addendum can be generated only after the terminal v5 failure record and
Objective-3 prerequisite are persisted, inside a clean `git archive` of the
exact definition parent, by
`tools/policy_cost_campaign_v6.py prepare-execution`. The workflow
