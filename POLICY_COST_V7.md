# Locked policy-frequency search cost v7

Version 6 passed preflight, every discovery/allocation barrier, and all 216
locked TRAIN-evaluation shards on GitHub run `33213087155`. Calibration then
failed before producing a decision because TRAIN shard 88, allocation 5632,
contained a legal semantic action whose complete-move probabilities all
underflowed to exact binary32 zero. The producer calculated its conditional
draw probability as `0/0` and serialized the JSON-invalid token `-nan`.
Independent download reproduced the artifact and SHA-256 exactly. This was a
deterministic evidence-serialization defect, not AI efficacy evidence.

All 21 v6 roots are retired. Version 7 changes only that proven defect: a
zero-mass semantic action receives canonical finite conditional draw mass zero,
the native self-test reproduces the exact edge case, and every TRAIN, SELECT,
and TEST producer must pass strict JSONL validation before upload. It uses
fresh `202705` roots and changes no controller
...
seed `202705140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202705100101`.
- SELECT: 32,768 matches, seed `202705100201`.
- TEST: 32,768 matches, seed `202705100301`.

All later roots use the same fresh v7 split suffixes: primary
`202705110101/0201/0301`, fresh confirmation
`202705120101/0201/0301`, truth `202705130101/0201/0301`, and maintained
counterfactual `202705160101/0201/0301`. Calibration uses `202705140101`,
SELECT bootstrap uses `202705150101`, safety uses
`202705210101/0102`, and final uses `202705220101/0102`.
The retired namespaces include all v1--v6 production and smoke roots,
...
and every `20270429` feasibility-smoke seed. The v7 definition
smoke `202705010101` and every `20270529` smoke seed are likewise burned and
...
plus both canonical v7 exclusion manifests (text and JSON), are bound in the
...
`202705140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v7.py train-input` into
...
max-t bootstrap with seed `202705150101`. The 1% floor must beat the 2% floor
...
real addendum can be generated only after the terminal v6 failure record and
Objective-3 prerequisite are persisted, inside a clean `git archive` of the
exact definition parent, by
`tools/policy_cost_campaign_v7.py prepare-execution`. The workflow
