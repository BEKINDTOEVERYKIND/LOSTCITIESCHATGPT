# Locked policy-frequency search cost v8

Version 7 passed preflight, every discovery/allocation barrier, and all 216
locked TRAIN-evaluation shards on GitHub run `33237653855`. Calibration then
failed before fitting or producing a decision: the native allocator's frozen
allocation-id schedule intentionally interleaves cells, the converter preserved
that order in `selected_units`, and the independent calibration boundary
required the identical 13,824 units in canonical cell/priority order. This was
a deterministic cross-module serialization-order defect, not AI efficacy
evidence.

All 21 v7 roots are retired. Version 8 changes only that proven defect: the
converter preserves allocation-id order for evaluator bindings while sorting
only the calibration manifest projection before sealing its digest. An exact
full-13,824-unit regression crosses the converter/calibration boundary. It uses
fresh `202707` roots and changes no controller
math, actor settings, sample size, model-adequacy requirement, TEST rule,
safety threshold, or final promotion gate. The terminal record is
...
seed `202707140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202707100101`.
- SELECT: 32,768 matches, seed `202707100201`.
- TEST: 32,768 matches, seed `202707100301`.

All later roots use the same fresh v8 split suffixes: primary
`202707110101/0201/0301`, fresh confirmation
`202707120101/0201/0301`, truth `202707130101/0201/0301`, and maintained
counterfactual `202707160101/0201/0301`. Calibration uses `202707140101`,
SELECT bootstrap uses `202707150101`, safety uses
`202707210101/0102`, and final uses `202707220101/0102`.
The retired namespaces include all v1--v7 production and smoke roots,
...
and every `20270529` feasibility-smoke seed. The v8 definition
smoke `202707010101` and every `20270729` smoke seed are likewise burned and
...
plus both canonical v8 exclusion manifests (text and JSON), are bound in the
...
`202707140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v8.py train-input` into
...
max-t bootstrap with seed `202707150101`. The 1% floor must beat the 2% floor
...
real addendum can be generated only after the terminal v7 failure record and
Objective-3 prerequisite are persisted, inside a clean `git archive` of the
exact definition parent, by
`tools/policy_cost_campaign_v8.py prepare-execution`. The workflow
