# Locked policy-frequency search cost v9

Version 8 passed preflight and every discovery/allocation barrier. Of its 216
locked TRAIN-evaluation slices on GitHub run `33257645061`, 215 completed. The
remaining slice never reached checkout or any campaign command because its
GitHub-hosted runner timed out while resolving pinned action metadata. The
complete-matrix barrier therefore stopped calibration before fitting or any
efficacy decision. This was external runner infrastructure failure, not AI
evidence.

All 21 v8 roots are retired. Version 9 changes only that proven exposure: each
GitHub runner evaluates four consecutive logical slices, while all 216 TRAIN
and all 192 SELECT/TEST slice boundaries, rows, budgets, filenames, reducers,
and adequacy requirements remain unchanged. It uses fresh `202708` roots and
...
seed `202708140101` and the exact binary64 floor `0x1p-150`. Its controller
...
- TRAIN: 65,536 matches, seed `202708100101`.
- SELECT: 32,768 matches, seed `202708100201`.
- TEST: 32,768 matches, seed `202708100301`.

All later roots use the same fresh v9 split suffixes: primary
`202708110101/0201/0301`, fresh confirmation
`202708120101/0201/0301`, truth `202708130101/0201/0301`, and maintained
counterfactual `202708160101/0201/0301`. Calibration uses `202708140101`,
SELECT bootstrap uses `202708150101`, safety uses
`202708210101/0102`, and final uses `202708220101/0102`.
The retired namespaces include all v1--v8 production and smoke roots,
...
and every `20270729` feasibility-smoke seed. The v9 definition
smoke `202708010101` and every `20270829` smoke seed are likewise burned and
...
plus both canonical v9 exclusion manifests (text and JSON), are bound in the
...
`202708140101`, standardized Huber loss (delta 1.345), SE floor 0.25, and the
...
The TRAIN TSV is converted once by `policy_cost_campaign_v9.py train-input` into
...
max-t bootstrap with seed `202708150101`. The 1% floor must beat the 2% floor
...
`tools/policy_cost_campaign_v9.py prepare-execution`. The workflow
