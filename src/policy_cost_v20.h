 ...
 * factorized into semantic card/action and conditional draw-source terms:
 *
 *   score(m) = beta * Q(m)
 ...
 * formed.  The current deployable schedule fixes alpha_action exactly zero.
 * It chooses the semantic action core by raw search Q, then applies only the
 * conditional-draw score within that chosen core.  Thus draw-policy
 * confidence cannot overrule search between strategically different moves.
 * The historical practical-effect hurdle remains a distinct legacy rollout
 * mechanism; v1 policy-cost actors bind its threshold to canonical +0 and
 * never fold it into this trusted selection.
 *
 ...
 */
#ifndef POLICY_COST_V20_H
#define POLICY_COST_V20_H
...
#define POLICY_COST_V20_VERSION 4U
...
#define POLICY_COST_V20_SOURCE_SEED UINT64_C(202702140101)
#define POLICY_COST_SOURCE_SEED UINT64_C(202807140101)
...
typedef struct PolicyCostTable {
    ...
    /* One round-shared predictive schedule.  beta is linearly interpolated at
     * the integer pre-move State.nply through ply 64.  In current v20 tables,
     * both alpha schedules use a disconnected early/late spline and are
     * exactly zero for plies 16..39; late interpolation begins at ply 40.
     * State.round remains diagnostic only.  The aliases retain source
     * compatibility for legacy-v1 table
 ...
 * implementation of hierarchical action-core leadership, same-core adjusted
 * evidence, and different-core raw evidence; the raw-world entry point below
 ...
 * Raw Q first chooses the semantic action core.  Within that core, the
 * adjusted leader must beat every same-core rival by z paired SEs and remain
 * raw-positive versus a higher conditional-draw prior.  It must beat every
 * different-core rival using raw paired search evidence.  Any malformed or
 * ambiguous panel returns candidate zero. */
