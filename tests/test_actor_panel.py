...
class LockedActorPanelTest(unittest.TestCase):
    def test_workflow_has_one_addendum_trigger_and_exact_locked_panels(self):
        ...
        self.assertIn(
            ...
            text)
        self.assertIn("ARENA_SIZE: '362592'", text)
        self.assertNotIn("make -C source", text)
        self.assertNotIn("path: source", text)
        # The evaluator binary is a historical campaign artifact whose hash
        # and size remain frozen above; current source trees intentionally do
        # not track generated executables.  The model remains a source input.
        root_bytes = TRANSPORT_ROOT.read_bytes()
