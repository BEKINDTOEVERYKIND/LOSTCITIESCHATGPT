...
MODULE_PATH = ROOT / "tools" / "policy_cost_exact17_v13.py"
SPEC = importlib.util.spec_from_file_location("policy_cost_exact17_v13", MODULE_PATH)
assert SPEC and SPEC.loader
exact17 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(exact17)

ALLOCATOR_PATH = ROOT / "tools" / "policy_cost_allocate_v13.py"
ALLOCATOR_SPEC = importlib.util.spec_from_file_location(
    "policy_cost_allocate_v13", ALLOCATOR_PATH
)
...
class PolicyCostExact17V4Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        subprocess.run([
            ...
            "tools/policy_cost_dataset_v13.c", *core[:-4],
            "src/policy_cost_v13.c", *core[-3:], "-lm", "-pthread",
        ], cwd=ROOT, check=True, capture_output=True, text=True)
        exclusions = (
            ROOT / "data" / "experiments" /
            "policy_cost_v13_exact17_exclusions.txt"
        )
    ...
    def test_exact_locked_case_set_exports_unique_orbits(self) -> None:
        ...
        self.assertEqual(
            ...
            "tools/policy_cost_dataset_v13.c",
        )
    ...
    def test_checked_in_pair_is_exactly_regenerated(self) -> None:
        text, evidence = exact17.build_outputs(ROOT, self.binary)
        checked_text = (
            ROOT / "data/experiments/policy_cost_v13_exact17_exclusions.txt"
        )
        checked_json = (
            ROOT / "data/experiments/policy_cost_v13_exact17_exclusions.json"
        )
    ...
    def test_native_producer_and_allocator_share_five_bin_union_contract(
        ...
        completed = subprocess.run([
            ...
            "--split", "TRAIN", "--smoke-seed", "202712290999",
    ...
    def test_contract_only_and_campaign_identities_are_disjoint(self) -> None:
        ...
        with self.assertRaisesRegex(
            allocator.AllocationError, "outside burned 20271229"
        ):
        ...
        validation_with_output = subprocess.run([
            *common, "--validate-contract-only", "--split", "TRAIN",
            "--smoke-seed", "202712290999", "--matches", "1",
