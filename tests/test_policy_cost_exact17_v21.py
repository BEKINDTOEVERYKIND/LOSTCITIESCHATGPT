...
MODULE_PATH = ROOT / "tools" / "policy_cost_exact17_v21.py"
SPEC = importlib.util.spec_from_file_location("policy_cost_exact17_v21", MODULE_PATH)
assert SPEC and SPEC.loader
exact17 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(exact17)

ALLOCATOR_PATH = ROOT / "tools" / "policy_cost_allocate_v21.py"
ALLOCATOR_SPEC = importlib.util.spec_from_file_location(
    "policy_cost_allocate_v21", ALLOCATOR_PATH
)
...
class PolicyCostExact17V4Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        subprocess.run([
            ...
            "tools/policy_cost_dataset_v21.c", *core[:-4],
            "src/policy_cost_v21.c", *core[-3:], "-lm", "-pthread",
    ...
    def test_exact_locked_case_set_exports_unique_orbits(self) -> None:
        ...
        self.assertEqual(
            ...
            "tools/policy_cost_dataset_v21.c",
        )
    ...
    def test_v21_reproduces_existing_canonical_exact17_cases(self) -> None:
        text, evidence = exact17.build_outputs(ROOT, self.binary)
        checked_text = (
    ...
    def test_native_producer_and_allocator_share_five_bin_union_contract(
        ...
        completed = subprocess.run([
            ...
            "--split", "TRAIN", "--smoke-seed", "202808290999",
    ...
    def test_contract_only_and_campaign_identities_are_disjoint(self) -> None:
        ...
        with self.assertRaisesRegex(
            allocator.AllocationError, "outside burned 20280829"
        ):
        ...
        validation_with_output = subprocess.run([
            *common, "--validate-contract-only", "--split", "TRAIN",
            "--smoke-seed", "202808290999", "--matches", "1",
