"""Fail-closed contracts for the fresh-seed policy-cost-v10 recovery."""
...
import yaml
from tools import policy_cost_allocate_v10 as allocator
from tools import policy_cost_calibration_v10 as calibration
from tools import policy_cost_campaign_v10 as campaign


ROOT = Path(__file__).resolve().parents[1]
V9_LAUNCH = "09d12bac0eec951997c78f7d1df616f9a1decc2c"
V9_PARENT = "95ee415828db20bd7e884e5207409cec4887f826"
V9_LAUNCH_TREE = "706be14d9b5eb5332cb52ba7c3e244d4ac0dee66"
...
V10_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v10_execution.json"
V10_WORKFLOW = ROOT / ".github/workflows/policy-cost-v10.yml"
V10_DEFINITION = ROOT / ".github/workflows/policy-cost-v10-definition.yml"
V9_FAILURE = ROOT / "data/experiments/policy_cost_v9_run_33278577188_failure.json"
V9_RETAIN = ROOT / "data/experiments/policy_cost_v9_infrastructure_retain.json"
...
class PolicyCostV10RecoveryTests(unittest.TestCase):
    def test_v9_terminal_record_is_complete_and_non_promotable(self) -> None:
        failure, retain = load(V9_FAILURE), load(V9_RETAIN)
        self.assertEqual(failure["schema"],
                         "lc-policy-cost-v9-burned-seed-binding-failure-v1")
        ...
        self.assertEqual(failure["github_run"], {
            ...
            "head_sha": V9_LAUNCH, "head_tree": V9_LAUNCH_TREE,
            "id": 33278577188,
            "url": "https://github.com/BEKINDTOEVERYKIND/"
                   "LOSTCITIESCHATGPT/actions/runs/33278577188",
        })
        cause = failure["cause"]
        self.assertEqual(cause["classification"],
                         "native_python_burned_seed_literal_mismatch")
        self.assertFalse(cause["efficacy_related"])
        self.assertEqual(cause["stage"]["job_id"], 99214015270)
        self.assertIn("before calibration fitting", cause["mechanism"])
        self.assertIn("216", cause["mechanism"])
        self.assertEqual(failure["job_disposition"], {
            "failure": 1, "skipped": 9, "success": 63,
            "total": 73, "train_evaluate_success": 54,
        })
        self.assertTrue(
            failure["fixed_seed_execution"]["all_policy_cost_v9_fixed_roots_retired"])
    ...
    def test_v9_launch_is_the_unique_direct_addendum(self) -> None:
        parent = subprocess.check_output(
            ["git", "rev-parse", f"{V9_LAUNCH}^"], cwd=ROOT, text=True).strip()
        tree = subprocess.check_output(
            ["git", "rev-parse", f"{V9_LAUNCH}^{{tree}}"], cwd=ROOT,
            text=True).strip()
        changed = subprocess.check_output(
            ["git", "diff-tree", "--no-commit-id", "--name-status", "-r", V9_LAUNCH],
            cwd=ROOT, text=True).strip()
        self.assertEqual(parent, V9_PARENT)
        self.assertEqual(tree, V9_LAUNCH_TREE)
        self.assertEqual(changed,
                         "A\tdata/experiments/locked_policy_cost_v9_execution.json")

    def test_every_v9_campaign_byte_is_immutable(self) -> None:
        paths = subprocess.check_output(
            ["git", "ls-tree", "-r", "--name-only", V9_LAUNCH],
        ...
        frozen = [path for path in paths if path.startswith(prefixes) or
                  ...
                  re.fullmatch(r"POLICY_COST_V[1-9]\.md", path)]
        self.assertIn("tools/policy_cost_dataset_v9.c", frozen)
        for relative in frozen:
            current = ROOT / relative
            self.assertTrue(current.is_file(), relative)
            expected = subprocess.check_output(
                ["git", "show", f"{V9_LAUNCH}:{relative}"], cwd=ROOT)
            self.assertEqual(current.read_bytes(), expected, relative)

    def test_all_twenty_one_v10_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202709") for root in roots))
        self.assertFalse(roots & previous)
        burned = plans[-1]["reservoirs"]["burned_source_seeds"]
        self.assertIn("all policy-cost-v9 fixed seeds", burned)
        self.assertIn("every 20270829 feasibility-smoke seed", burned)

    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v10.c").read_text(encoding="utf-8")
    ...
    def test_v10_preserves_every_efficacy_design_and_gate(self) -> None:
        v9, v10 = load(V9_PLAN), load(V10_PLAN)
        cal8, cal9 = copy.deepcopy(v9["calibration"]), copy.deepcopy(v10["calibration"])
        cal8["schedule_seed"] = cal9["schedule_seed"]
        cal8["cross_validation"]["seed"] = cal9["cross_validation"]["seed"]
        self.assertEqual(cal8, cal9)
        ctl8, ctl9 = copy.deepcopy(v9["controller"]), copy.deepcopy(v10["controller"])
        ctl8["artifact_binding"]["source_seed"] = ctl9["artifact_binding"]["source_seed"]
        self.assertEqual(ctl8, ctl9)
        self.assertEqual(v9["multiplicity"], v10["multiplicity"])
        self.assertEqual(v9["prerequisite"], v10["prerequisite"])
        self.assertEqual(v9["test"], v10["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v9[stage], "seeds"), without(v10[stage], "seeds"))
        self.assertEqual(without(v9["selection"], "bootstrap"),
                         without(v10["selection"], "bootstrap"))
        self.assertEqual(without(v9["selection"]["bootstrap"], "seed"),
                         without(v10["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v9["reservoirs"][key], v10["reservoirs"][key])
        self.assertEqual(seed_agnostic(v9["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v10["reservoirs"]["search_and_truth"]))
        self.assertEqual(
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V10_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V10_EXECUTION.exists())
        workflow = yaml.safe_load(V10_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v10.c", text)
        self.assertIn("tools/policy_cost_campaign_v10.py", text)
        self.assertIn("locked_policy_cost_v9_execution.json", text)
        self.assertIn("policy_cost_v9_run_33278577188_failure.json", text)
        self.assertIn("bin/policy_cost_dataset self-test", text)
        definition = V10_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v10.c", definition)
        self.assertIn("tests/test_policy_cost_v10_recovery.py", definition)
