"""Fail-closed contracts for the fresh-seed policy-cost-v12 recovery."""
...
import yaml
from tools import policy_cost_allocate_v12 as allocator
from tools import policy_cost_calibration_v12 as calibration
from tools import policy_cost_campaign_v12 as campaign
...
V12_PLAN = ROOT / "data/experiments/locked_policy_cost_v12_plan.json"
V12_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v12_execution.json"
V12_WORKFLOW = ROOT / ".github/workflows/policy-cost-v12.yml"
V12_DEFINITION = ROOT / ".github/workflows/policy-cost-v12-definition.yml"
...
class PolicyCostV12RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v12_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202711") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v12.c").read_text(encoding="utf-8")
    ...
    def test_v12_preserves_every_efficacy_design_and_gate(self) -> None:
        v11, v12 = load(V11_PLAN), load(V12_PLAN)
        cal8, cal9 = copy.deepcopy(v11["calibration"]), copy.deepcopy(v12["calibration"])
        cal8["schedule_seed"] = cal9["schedule_seed"]
        cal8["cross_validation"]["seed"] = cal9["cross_validation"]["seed"]
        self.assertEqual(cal8, cal9)
        ctl8, ctl9 = copy.deepcopy(v11["controller"]), copy.deepcopy(v12["controller"])
        ctl8["artifact_binding"]["source_seed"] = ctl9["artifact_binding"]["source_seed"]
        self.assertEqual(ctl8, ctl9)
        self.assertEqual(v11["multiplicity"], v12["multiplicity"])
        self.assertEqual(v11["prerequisite"], v12["prerequisite"])
        self.assertEqual(v11["test"], v12["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v11[stage], "seeds"), without(v12[stage], "seeds"))
        self.assertEqual(without(v11["selection"], "bootstrap"),
                         without(v12["selection"], "bootstrap"))
        self.assertEqual(without(v11["selection"]["bootstrap"], "seed"),
                         without(v12["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v11["reservoirs"][key], v12["reservoirs"][key])
        self.assertEqual(seed_agnostic(v11["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v12["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v11["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v12["reservoirs"]["discovery"], "seeds", "sealed_smokes"))

    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V12_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V12_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V12_EXECUTION.exists())
        workflow = yaml.safe_load(V12_WORKFLOW.read_text(encoding="utf-8"))
        self.assertEqual(workflow[True]["push"]["paths"],
                         ["data/experiments/locked_policy_cost_v12_execution.json"])
        self.assertFalse(workflow["concurrency"]["cancel-in-progress"])
        text = V12_WORKFLOW.read_text(encoding="utf-8")
        self.assertNotIn("workflow_dispatch", text)
        self.assertIn("test \"$GITHUB_RUN_ATTEMPT\" = 1", text)
        self.assertIn("tools/policy_cost_dataset_v12.c", text)
        self.assertIn("tools/policy_cost_campaign_v12.py", text)
        ...
        definition = V12_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v12.c", definition)
        self.assertIn("tests/test_policy_cost_v12_recovery.py", definition)
