"""Fail-closed contracts for the fresh-seed policy-cost-v14 recovery."""
...
import yaml
from tools import policy_cost_allocate_v14 as allocator
from tools import policy_cost_calibration_v14 as calibration
from tools import policy_cost_campaign_v14 as campaign
...
V14_PLAN = ROOT / "data/experiments/locked_policy_cost_v14_plan.json"
V14_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v14_execution.json"
V14_WORKFLOW = ROOT / ".github/workflows/policy-cost-v14.yml"
V14_DEFINITION = ROOT / ".github/workflows/policy-cost-v14-definition.yml"
...
class PolicyCostV14RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v14_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202801") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v14.c").read_text(encoding="utf-8")
    ...
    def test_v14_preserves_every_efficacy_design_and_gate(self) -> None:
        v11, v14 = load(V11_PLAN), load(V14_PLAN)
        cal8, cal9 = copy.deepcopy(v11["calibration"]), copy.deepcopy(v14["calibration"])
        cal8["schedule_seed"] = cal9["schedule_seed"]
        cal8["cross_validation"]["seed"] = cal9["cross_validation"]["seed"]
        self.assertEqual(cal8, cal9)
        ctl8, ctl9 = copy.deepcopy(v11["controller"]), copy.deepcopy(v14["controller"])
        ctl8["artifact_binding"]["source_seed"] = ctl9["artifact_binding"]["source_seed"]
        self.assertEqual(ctl8, ctl9)
        self.assertEqual(v11["multiplicity"], v14["multiplicity"])
        self.assertEqual(v11["prerequisite"], v14["prerequisite"])
        self.assertEqual(v11["test"], v14["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v11[stage], "seeds"), without(v14[stage], "seeds"))
        self.assertEqual(without(v11["selection"], "bootstrap"),
                         without(v14["selection"], "bootstrap"))
        self.assertEqual(without(v11["selection"]["bootstrap"], "seed"),
                         without(v14["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v11["reservoirs"][key], v14["reservoirs"][key])
        self.assertEqual(seed_agnostic(v11["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v14["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v11["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v14["reservoirs"]["discovery"], "seeds", "sealed_smokes"))

    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V14_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V14_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V14_EXECUTION.exists())
        workflow = yaml.safe_load(V14_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v14.c", text)
        self.assertIn("tools/policy_cost_campaign_v14.py", text)
        self.assertIn("locked_policy_cost_v13_execution.json", text)
        self.assertIn("policy_cost_v13_run_33350120459_failure.json", text)
        ...
        definition = V14_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v14.c", definition)
        self.assertIn("tests/test_policy_cost_v14_recovery.py", definition)
