"""Fail-closed contracts for the fresh-seed policy-cost-v15 recovery."""
...
import yaml
from tools import policy_cost_allocate_v15 as allocator
from tools import policy_cost_calibration_v15 as calibration
from tools import policy_cost_campaign_v15 as campaign
...
V15_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v15_execution.json"
V15_WORKFLOW = ROOT / ".github/workflows/policy-cost-v15.yml"
V15_DEFINITION = ROOT / ".github/workflows/policy-cost-v15-definition.yml"
...
class PolicyCostV15RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v15_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202802") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v15.c").read_text(encoding="utf-8")
    ...
    def test_v15_preserves_every_efficacy_design_and_gate(self) -> None:
        v11, v15 = load(V11_PLAN), load(V15_PLAN)
        cal8, cal9 = copy.deepcopy(v11["calibration"]), copy.deepcopy(v15["calibration"])
        support = cal9["model_lack"].pop("cell_saturated_support")
        self.assertIn("without consulting held-out outcomes", support)
        self.assertIn("omit conditional-draw", support)
        cal8["schedule_seed"] = cal9["schedule_seed"]
        cal8["cross_validation"]["seed"] = cal9["cross_validation"]["seed"]
        self.assertEqual(cal8, cal9)
        ctl8, ctl9 = copy.deepcopy(v11["controller"]), copy.deepcopy(v15["controller"])
        ctl8["artifact_binding"]["source_seed"] = ctl9["artifact_binding"]["source_seed"]
        self.assertEqual(ctl8, ctl9)
        self.assertEqual(v11["multiplicity"], v15["multiplicity"])
        self.assertEqual(v11["prerequisite"], v15["prerequisite"])
        self.assertEqual(v11["test"], v15["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v11[stage], "seeds"), without(v15[stage], "seeds"))
        self.assertEqual(without(v11["selection"], "bootstrap"),
                         without(v15["selection"], "bootstrap"))
        self.assertEqual(without(v11["selection"]["bootstrap"], "seed"),
                         without(v15["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v11["reservoirs"][key], v15["reservoirs"][key])
        self.assertEqual(seed_agnostic(v11["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v15["reservoirs"]["search_and_truth"]))
        self.assertEqual(
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V15_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V15_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V15_EXECUTION.exists())
        workflow = yaml.safe_load(V15_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v15.c", text)
        self.assertIn("tools/policy_cost_campaign_v15.py", text)
        self.assertIn("locked_policy_cost_v14_execution.json", text)
        self.assertIn("policy_cost_v14_run_33360219428_failure.json", text)
        ...
        definition = V15_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v15.c", definition)
        self.assertIn("tests/test_policy_cost_v15_recovery.py", definition)
