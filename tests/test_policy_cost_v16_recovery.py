"""Fail-closed contracts for the fresh-seed policy-cost-v16 recovery."""
...
import yaml
from tools import policy_cost_allocate_v16 as allocator
from tools import policy_cost_calibration_v16 as calibration
from tools import policy_cost_campaign_v16 as campaign
...
V15_PLAN = ROOT / "data/experiments/locked_policy_cost_v15_plan.json"
V16_PLAN = ROOT / "data/experiments/locked_policy_cost_v16_plan.json"
V16_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v16_execution.json"
V16_WORKFLOW = ROOT / ".github/workflows/policy-cost-v16.yml"
V16_DEFINITION = ROOT / ".github/workflows/policy-cost-v16-definition.yml"
...
V15_FAILURE = ROOT / "data/experiments/policy_cost_v15_run_33435987783_failure.json"
...
class PolicyCostV16RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v16_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202803") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v16.c").read_text(encoding="utf-8")
    ...
    def test_v16_preserves_every_efficacy_design_and_gate(self) -> None:
        v15, v16 = load(V15_PLAN), load(V16_PLAN)
        cal15 = copy.deepcopy(v15["calibration"])
        cal16 = copy.deepcopy(v16["calibration"])
        self.assertEqual(cal15["solver"]["max_irls_iterations"], 500)
        self.assertEqual(cal16["solver"]["max_irls_iterations"], 2000)
        cal15["solver"]["max_irls_iterations"] = 2000
        cal15["schedule_seed"] = cal16["schedule_seed"]
        cal15["cross_validation"]["seed"] = cal16["cross_validation"]["seed"]
        self.assertEqual(cal15, cal16)
        ctl15 = copy.deepcopy(v15["controller"])
        ctl16 = copy.deepcopy(v16["controller"])
        ctl15["artifact_binding"]["source_seed"] = ctl16["artifact_binding"]["source_seed"]
        self.assertEqual(ctl15, ctl16)
        self.assertEqual(v15["multiplicity"], v16["multiplicity"])
        self.assertEqual(v15["prerequisite"], v16["prerequisite"])
        self.assertEqual(v15["test"], v16["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v15[stage], "seeds"), without(v16[stage], "seeds"))
        self.assertEqual(without(v15["selection"], "bootstrap"),
                         without(v16["selection"], "bootstrap"))
        self.assertEqual(without(v15["selection"]["bootstrap"], "seed"),
                         without(v16["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v15["reservoirs"][key], v16["reservoirs"][key])
        self.assertEqual(seed_agnostic(v15["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v16["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v15["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v16["reservoirs"]["discovery"], "seeds", "sealed_smokes"))

    def test_v15_terminal_record_and_solver_recovery_are_fail_closed(self) -> None:
        failure = load(V15_FAILURE)
        self.assertEqual(
            failure["schema"],
            "lc-policy-cost-v15-irls-iteration-cap-exhaustion-v1",
        )
        self.assertEqual(failure["status"], "complete_terminal_non_promotable")
        self.assertFalse(failure["partial_efficacy_inspected"])
        self.assertFalse(failure["promotion_efficacy_data_observed"])
        self.assertFalse(failure["maintained_actor_changed"])
        self.assertEqual(failure["github_run"]["id"], 33435987783)
        self.assertEqual(failure["github_run"]["attempt"], 1)
        self.assertEqual(failure["cause"]["stage"]["job_id"], 99730964215)
        self.assertTrue(
            failure["fixed_seed_execution"]["all_policy_cost_v15_fixed_roots_retired"]
        )
        self.assertTrue(failure["fixed_seed_execution"]["retry_forbidden"])
        self.assertEqual(failure["immutable_retention"]["train_evaluation_batch_count"], 54)
        self.assertEqual(failure["immutable_retention"]["train_logical_slice_count"], 216)
        diagnostic = failure["recovery_diagnostic"]
        self.assertEqual(diagnostic["frozen_cap"], 500)
        self.assertEqual(diagnostic["coefficient_convergence_iteration"], 754)
        self.assertEqual(diagnostic["full_train_calibration_completed_with_cap"], 2000)
        self.assertFalse(diagnostic["strength_or_gate_result_inspected"])
        self.assertEqual(
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V16_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V16_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V16_EXECUTION.exists())
        workflow = yaml.safe_load(V16_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v16.c", text)
        self.assertIn("tools/policy_cost_campaign_v16.py", text)
        self.assertIn("locked_policy_cost_v15_execution.json", text)
        self.assertIn("policy_cost_v15_run_33435987783_failure.json", text)
        ...
        definition = V16_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v16.c", definition)
        self.assertIn("tests/test_policy_cost_v16_recovery.py", definition)
