"""Fail-closed contracts for the fresh-seed phase-selective policy-cost v18."""
...
import yaml
from tools import policy_cost_allocate_v18 as allocator
from tools import policy_cost_calibration_v18 as calibration
from tools import policy_cost_campaign_v18 as campaign
...
V18_WORKFLOW = ROOT / ".github/workflows/policy-cost-v18.yml"
V18_DEFINITION = ROOT / ".github/workflows/policy-cost-v18-definition.yml"
...
V17_FAILURE = ROOT / "data/experiments/policy_cost_v17_run_33629259360_failure.json"
...
class PolicyCostV18RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v18_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202805") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v18.c").read_text(encoding="utf-8")
    ...
    def test_v18_changes_only_preregistered_controller_semantics_and_fit(self) -> None:
        v16, v18 = load(V16_PLAN), load(V18_PLAN)
        self.assertEqual(v16["calibration"]["anchors"], v18["calibration"]["anchors"])
        self.assertEqual(v16["calibration"]["solver"], v18["calibration"]["solver"])
        self.assertEqual(v16["calibration"]["smoothness_grid"],
                         v18["calibration"]["smoothness_grid"])
        self.assertEqual(v16["calibration"]["huber_delta"],
                         v18["calibration"]["huber_delta"])
        self.assertEqual(v16["calibration"]["standard_error_floor"],
                         v18["calibration"]["standard_error_floor"])
        self.assertEqual(v16["calibration"]["negative_evidence_contract"],
                         v18["calibration"]["negative_evidence_contract"])
        self.assertIn("fixed exactly zero for 16 <= ply < 40",
                      v18["calibration"]["coefficient_constraints"])
        self.assertIn("unrestricted_full_spline",
                      next(key for key in v18["calibration"]["model_lack"]
                           if key.startswith("reject_if_")))
        ctl16 = copy.deepcopy(v16["controller"])
        ctl17 = copy.deepcopy(v18["controller"])
        ...
        self.assertEqual(v16["multiplicity"], v18["multiplicity"])
        self.assertEqual(v16["prerequisite"], v18["prerequisite"])
        self.assertEqual(v16["test"], v18["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v16[stage], "seeds"), without(v18[stage], "seeds"))
        self.assertEqual(without(v16["selection"], "bootstrap"),
                         without(v18["selection"], "bootstrap"))
        self.assertEqual(without(v16["selection"]["bootstrap"], "seed"),
                         without(v18["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v16["reservoirs"][key], v18["reservoirs"][key])
        self.assertEqual(seed_agnostic(v16["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v18["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v16["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v18["reservoirs"]["discovery"], "seeds", "sealed_smokes"))
    ...
    def test_v17_preflight_failure_is_terminal_and_bound(self) -> None:
        failure = load(V17_FAILURE)
        self.assertEqual(
            failure["schema"],
            "lc-policy-cost-v17-preflight-versioned-runtime-validation-scope-failure-v1",
        )
        self.assertEqual(failure["github_run"]["id"], 33629259360)
        self.assertFalse(failure["fixed_seed_execution"]["campaign_seed_or_truth_use_began"])
        self.assertTrue(failure["fixed_seed_execution"]["all_policy_cost_v17_fixed_roots_retired"])
        self.assertTrue(failure["fixed_seed_execution"]["retry_forbidden"])
        self.assertFalse(failure["partial_efficacy_inspected"])
        self.assertFalse(failure["maintained_actor_changed"])
        self.assertEqual(failure["execution"]["sha256"], digest(V17_EXECUTION))
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V18_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V18_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V18_EXECUTION.exists())
        workflow = yaml.safe_load(V18_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v18.c", text)
        self.assertIn("tools/policy_cost_campaign_v18.py", text)
        self.assertIn("locked_policy_cost_v17_execution.json", text)
        self.assertIn("policy_cost_v17_run_33629259360_failure.json", text)
        self.assertNotIn("bin/test_policy_cost\n", text.split(
            "cp src/policy_cost_v18.c src/policy_cost.c", 1
        ...
        definition = V18_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v18.c", definition)
        self.assertIn("tests/test_policy_cost_v18_recovery.py", definition)
        ...
        self.assertIn("bin/test_policy_cost_v18", versioned_native)
        self.assertNotIn("bin/test_policy_cost\n", versioned_native)
