"""Fail-closed contracts for the fresh-seed phase-selective policy-cost v19."""
...
import yaml
from tools import policy_cost_allocate_v19 as allocator
from tools import policy_cost_calibration_v19 as calibration
from tools import policy_cost_campaign_v19 as campaign
...
V19_WORKFLOW = ROOT / ".github/workflows/policy-cost-v19.yml"
V19_DEFINITION = ROOT / ".github/workflows/policy-cost-v19-definition.yml"
...
class PolicyCostV19RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v19_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202806") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v19.c").read_text(encoding="utf-8")
    ...
    def test_v19_changes_only_preregistered_controller_semantics_and_fit(self) -> None:
        v16, v19 = load(V16_PLAN), load(V19_PLAN)
        self.assertEqual(v16["calibration"]["anchors"], v19["calibration"]["anchors"])
        self.assertEqual(v16["calibration"]["solver"], v19["calibration"]["solver"])
        self.assertEqual(v16["calibration"]["smoothness_grid"],
                         v19["calibration"]["smoothness_grid"])
        self.assertEqual(v16["calibration"]["huber_delta"],
                         v19["calibration"]["huber_delta"])
        self.assertEqual(v16["calibration"]["standard_error_floor"],
                         v19["calibration"]["standard_error_floor"])
        self.assertEqual(v16["calibration"]["negative_evidence_contract"],
                         v19["calibration"]["negative_evidence_contract"])
        self.assertIn("fixed exactly zero for 16 <= ply < 40",
                      v19["calibration"]["coefficient_constraints"])
        self.assertIn("unrestricted_full_spline",
                      next(key for key in v19["calibration"]["model_lack"]
                           if key.startswith("reject_if_")))
        ctl16 = copy.deepcopy(v16["controller"])
        ctl17 = copy.deepcopy(v19["controller"])
        ...
        self.assertEqual(v16["multiplicity"], v19["multiplicity"])
        self.assertEqual(v16["prerequisite"], v19["prerequisite"])
        self.assertEqual(v16["test"], v19["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v16[stage], "seeds"), without(v19[stage], "seeds"))
        self.assertEqual(without(v16["selection"], "bootstrap"),
                         without(v19["selection"], "bootstrap"))
        self.assertEqual(without(v16["selection"]["bootstrap"], "seed"),
                         without(v19["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v16["reservoirs"][key], v19["reservoirs"][key])
        self.assertEqual(seed_agnostic(v16["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v19["reservoirs"]["search_and_truth"]))
        self.assertEqual(
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V19_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V19_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V19_EXECUTION.exists())
        workflow = yaml.safe_load(V19_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v19.c", text)
        self.assertIn("tools/policy_cost_campaign_v19.py", text)
        self.assertIn("locked_policy_cost_v18_execution.json", text)
        self.assertIn("policy_cost_v18_run_33659346537_failure.json", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.txt", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.json", text)
        self.assertNotIn("bindings/exact17/policy_cost_v19_exact17", text)
        self.assertNotIn("bin/test_policy_cost\n", text.split(
            "cp src/policy_cost_v19.c src/policy_cost.c", 1
        ...
        definition = V19_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v19.c", definition)
        self.assertIn("tests/test_policy_cost_v19_recovery.py", definition)
        ...
        self.assertIn("bin/test_policy_cost_v19", versioned_native)
        self.assertNotIn("bin/test_policy_cost\n", versioned_native)
