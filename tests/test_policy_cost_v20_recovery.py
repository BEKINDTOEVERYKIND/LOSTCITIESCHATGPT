"""Fail-closed contracts for the fresh-seed phase-selective policy-cost v20."""
...
import yaml
from tools import policy_cost_allocate_v20 as allocator
from tools import policy_cost_calibration_v20 as calibration
from tools import policy_cost_campaign_v20 as campaign
...
V20_WORKFLOW = ROOT / ".github/workflows/policy-cost-v20.yml"
V20_DEFINITION = ROOT / ".github/workflows/policy-cost-v20-definition.yml"
...
class PolicyCostV20RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v20_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202807") for root in roots))
    ...
    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v20.c").read_text(encoding="utf-8")
    ...
    def test_v20_preserves_v19_actor_samples_and_gates_exactly(self) -> None:
        v19, v20 = load(V19_PLAN), load(V20_PLAN)
        self.assertEqual(v19["calibration"]["anchors"], v20["calibration"]["anchors"])
        self.assertEqual(v19["calibration"]["solver"], v20["calibration"]["solver"])
        self.assertEqual(v19["calibration"]["smoothness_grid"],
                         v20["calibration"]["smoothness_grid"])
        self.assertEqual(v19["calibration"]["huber_delta"],
                         v20["calibration"]["huber_delta"])
        self.assertEqual(v19["calibration"]["standard_error_floor"],
                         v20["calibration"]["standard_error_floor"])
        self.assertEqual(v19["calibration"]["negative_evidence_contract"],
                         v20["calibration"]["negative_evidence_contract"])
        self.assertIn("fixed exactly zero for 16 <= ply < 40",
                      v20["calibration"]["coefficient_constraints"])
        self.assertIn("unrestricted_full_spline",
                      next(key for key in v20["calibration"]["model_lack"]
                           if key.startswith("reject_if_")))
        self.assertEqual(seed_agnostic(v19["controller"]),
                         seed_agnostic(v20["controller"]))
        self.assertEqual(v19["multiplicity"], v20["multiplicity"])
        self.assertEqual(v19["prerequisite"], v20["prerequisite"])
        self.assertEqual(v19["test"], v20["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v19[stage], "seeds"), without(v20[stage], "seeds"))
        self.assertEqual(without(v19["selection"], "bootstrap"),
                         without(v20["selection"], "bootstrap"))
        self.assertEqual(without(v19["selection"]["bootstrap"], "seed"),
                         without(v20["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v19["reservoirs"][key], v20["reservoirs"][key])
        self.assertEqual(seed_agnostic(v19["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v20["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v19["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V20_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V20_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V20_EXECUTION.exists())
        workflow = yaml.safe_load(V20_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v20.c", text)
        self.assertIn("tools/policy_cost_campaign_v20.py", text)
        self.assertIn("locked_policy_cost_v19_execution.json", text)
        self.assertIn("policy_cost_v19_run_33678694075_failure.json", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.txt", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.json", text)
        self.assertNotIn("bindings/exact17/policy_cost_v20_exact17", text)
        self.assertNotIn("bin/test_policy_cost\n", text.split(
            "cp src/policy_cost_v20.c src/policy_cost.c", 1
        ...
        definition = V20_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v20.c", definition)
        self.assertIn("tests/test_policy_cost_v20_recovery.py", definition)
        ...
        self.assertIn("bin/test_policy_cost_v20", versioned_native)
        self.assertNotIn("bin/test_policy_cost\n", versioned_native)
