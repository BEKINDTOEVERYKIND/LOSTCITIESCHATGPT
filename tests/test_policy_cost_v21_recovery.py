"""Fail-closed contracts for the fresh-seed phase-selective policy-cost v21."""
...
import yaml
from tools import policy_cost_allocate_v21 as allocator
from tools import policy_cost_calibration_v21 as calibration
from tools import policy_cost_campaign_v21 as campaign
...
V18_PLAN = ROOT / "data/experiments/locked_policy_cost_v18_plan.json"
V19_PLAN = ROOT / "data/experiments/locked_policy_cost_v19_plan.json"
V20_PLAN = ROOT / "data/experiments/locked_policy_cost_v20_plan.json"
V21_PLAN = ROOT / "data/experiments/locked_policy_cost_v21_plan.json"
V21_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v21_execution.json"
V21_WORKFLOW = ROOT / ".github/workflows/policy-cost-v21.yml"
V21_DEFINITION = ROOT / ".github/workflows/policy-cost-v21-definition.yml"
...
V18_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v18_execution.json"
V18_FAILURE = ROOT / "data/experiments/policy_cost_v18_run_33659346537_failure.json"
V19_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v19_execution.json"
V19_FAILURE = ROOT / "data/experiments/policy_cost_v19_run_33678694075_failure.json"
V20_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v20_execution.json"
V20_FAILURE = ROOT / "data/experiments/policy_cost_v20_run_33718162703_failure.json"
...
class PolicyCostV21RecoveryTests(unittest.TestCase):
    @staticmethod
    ...
    def test_all_twenty_one_v21_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202808") for root in roots))
        ...
        self.assertIn("all policy-cost-v17 fixed seeds", burned)
        self.assertIn("every 20280429 feasibility-smoke seed", burned)
        self.assertIn("all policy-cost-v18 fixed seeds", burned)
        self.assertIn("every 20280529 feasibility-smoke seed", burned)
        self.assertIn("all policy-cost-v19 fixed seeds", burned)
        self.assertIn("every 20280629 feasibility-smoke seed", burned)
        self.assertIn("all policy-cost-v20 fixed seeds", burned)
        self.assertIn("every 20280729 feasibility-smoke seed", burned)

    def test_native_evaluator_and_python_consumer_share_burned_seed_literal(self) -> None:
        source = (ROOT / "tools/policy_cost_dataset_v21.c").read_text(encoding="utf-8")
    ...
    def test_v21_preserves_v20_actor_samples_and_gates_exactly(self) -> None:
        v20, v21 = load(V20_PLAN), load(V21_PLAN)
        self.assertEqual(v20["calibration"]["anchors"], v21["calibration"]["anchors"])
        self.assertEqual(v20["calibration"]["solver"], v21["calibration"]["solver"])
        self.assertEqual(v20["calibration"]["smoothness_grid"],
                         v21["calibration"]["smoothness_grid"])
        self.assertEqual(v20["calibration"]["huber_delta"],
                         v21["calibration"]["huber_delta"])
        self.assertEqual(v20["calibration"]["standard_error_floor"],
                         v21["calibration"]["standard_error_floor"])
        self.assertEqual(v20["calibration"]["negative_evidence_contract"],
                         v21["calibration"]["negative_evidence_contract"])
        self.assertIn("fixed exactly zero for 16 <= ply < 40",
                      v21["calibration"]["coefficient_constraints"])
        self.assertIn("unrestricted_full_spline",
                      next(key for key in v21["calibration"]["model_lack"]
                           if key.startswith("reject_if_")))
        self.assertEqual(seed_agnostic(v20["controller"]),
                         seed_agnostic(v21["controller"]))
        self.assertEqual(v20["multiplicity"], v21["multiplicity"])
        self.assertEqual(v20["prerequisite"], v21["prerequisite"])
        self.assertEqual(v20["test"], v21["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v20[stage], "seeds"), without(v21[stage], "seeds"))
        self.assertEqual(without(v20["selection"], "bootstrap"),
                         without(v21["selection"], "bootstrap"))
        self.assertEqual(without(v20["selection"]["bootstrap"], "seed"),
                         without(v21["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v20["reservoirs"][key], v21["reservoirs"][key])
        self.assertEqual(seed_agnostic(v20["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v21["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v20["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v21["reservoirs"]["discovery"], "seeds", "sealed_smokes"))

    def test_pair_product_cancellation_repair_is_32_ulp_bounded(self) -> None:
        pair = {
            "a": 0, "b": 1,
            "delta_a_minus_b": 51.6000000000001,
            "paired_se": 4.400558405033424e-15,
            "sum_products": 0.0,
        }
        means = [0.0, -51.6000000000001]
        sums = [0.0, 1363230.719999999]
        campaign._pair_moment(pair, means, sums, 512, "sealed v20 witness")
        corrupted = dict(pair)
        corrupted["sum_products"] = 1.0e-7
        with self.assertRaisesRegex(campaign.EvidenceError,
                                    "sum_products algebra drift"):
            campaign._pair_moment(
                corrupted, means, sums, 512, "material corruption"
            )

    def test_v20_train_cancellation_failure_is_terminal_and_bound(self) -> None:
        failure = load(V20_FAILURE)
        self.assertEqual(
            failure["schema"],
            "lc-policy-cost-v20-train-pair-cancellation-roundoff-failure-v1",
        )
        self.assertEqual(failure["github_run"]["id"], 33718162703)
        self.assertEqual(failure["github_run"]["train_evaluate_success"], 54)
        self.assertEqual(failure["cause"]["native_sum_products"], 0.0)
        self.assertEqual(
            failure["cause"]["python_reconstructed_sum_products"],
            -3.14321368932724e-09,
        )
        self.assertFalse(failure["partial_efficacy_inspected"])
        self.assertFalse(failure["promotion_efficacy_data_observed"])
        self.assertTrue(
            failure["fixed_seed_execution"]["all_policy_cost_v20_fixed_roots_retired"]
        )
        self.assertEqual(failure["execution"]["sha256"], digest(V20_EXECUTION))

    def test_binary64_round_trip_repair_is_one_ulp_and_structure_bounded(self) -> None:
        value = 0.863224838051126
        adjacent = math.nextafter(value, math.inf)
        two_ulps = math.nextafter(adjacent, math.inf)
        sealed = [{"ply": 19, "beta_search": value, "nested": [0.0, 1.0]}]
        recomputed = copy.deepcopy(sealed)
        recomputed[0]["beta_search"] = adjacent
        self.assertTrue(campaign._same_derived_threshold_tree(sealed, recomputed))
        recomputed[0]["beta_search"] = two_ulps
        self.assertFalse(campaign._same_derived_threshold_tree(sealed, recomputed))
        recomputed = copy.deepcopy(sealed)
        recomputed[0]["nested"].append(2.0)
        self.assertFalse(campaign._same_derived_threshold_tree(sealed, recomputed))

    def test_v19_select_transport_failure_is_terminal_and_bound(self) -> None:
        failure = load(V19_FAILURE)
        self.assertEqual(
            failure["schema"],
            "lc-policy-cost-v19-select-calibration-binary64-round-trip-failure-v1",
        )
        self.assertEqual(failure["github_run"]["id"], 33678694075)
        self.assertEqual(failure["github_run"]["job_disposition"]["train_evaluate_success"], 54)
        self.assertEqual(failure["github_run"]["job_disposition"]["select_evaluate_failure"], 48)
        self.assertTrue(failure["immutable_calibration_artifact"]["calibration_gate_passed"])
        self.assertEqual(failure["cause"]["maximum_absolute_difference"], 2.0 ** -53)
        self.assertEqual(failure["cause"]["mismatched_finite_value_count"], 17)
        self.assertFalse(failure["partial_efficacy_inspected"])
        self.assertFalse(failure["promotion_efficacy_data_observed"])
        self.assertTrue(failure["fixed_seed_execution"]["all_policy_cost_v19_fixed_roots_retired"])
        self.assertTrue(failure["fixed_seed_execution"]["retry_forbidden"])
        self.assertEqual(failure["execution"]["sha256"], digest(V19_EXECUTION))
    ...
    def test_v18_preflight_failure_is_terminal_and_bound(self) -> None:
        failure = load(V18_FAILURE)
        self.assertEqual(
            failure["schema"],
            "lc-policy-cost-v18-preflight-exact17-transport-filename-failure-v1",
        )
        self.assertEqual(failure["github_run"]["id"], 33659346537)
        self.assertFalse(failure["fixed_seed_execution"]["campaign_seed_or_truth_use_began"])
        self.assertTrue(failure["fixed_seed_execution"]["all_policy_cost_v18_fixed_roots_retired"])
        self.assertTrue(failure["fixed_seed_execution"]["retry_forbidden"])
        self.assertFalse(failure["partial_efficacy_inspected"])
        self.assertFalse(failure["maintained_actor_changed"])
        self.assertEqual(failure["execution"]["sha256"], digest(V18_EXECUTION))
    ...
    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V21_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
        ...
        self.assertEqual(jobs["test_evaluate"]["strategy"]["matrix"]["batch"],
                         list(range(48)))
        text = V21_WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V21_EXECUTION.exists())
        workflow = yaml.safe_load(V21_WORKFLOW.read_text(encoding="utf-8"))
        self.assertEqual(workflow[True]["push"]["paths"],
                         ["data/experiments/locked_policy_cost_v21_execution.json"])
        self.assertFalse(workflow["concurrency"]["cancel-in-progress"])
        text = V21_WORKFLOW.read_text(encoding="utf-8")
        self.assertNotIn("workflow_dispatch", text)
        self.assertIn("test \"$GITHUB_RUN_ATTEMPT\" = 1", text)
        self.assertIn("tools/policy_cost_dataset_v21.c", text)
        self.assertIn("tools/policy_cost_campaign_v21.py", text)
        self.assertIn("locked_policy_cost_v20_execution.json", text)
        self.assertIn("policy_cost_v20_run_33718162703_failure.json", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.txt", text)
        self.assertIn("policy_cost_v16_exact17_exclusions.json", text)
        self.assertNotIn("bindings/exact17/policy_cost_v21_exact17", text)
        self.assertNotIn("bin/test_policy_cost\n", text.split(
            "cp src/policy_cost_v21.c src/policy_cost.c", 1
        )[1].split("Freeze a source-free transport", 1)[0])
        ...
        definition = V21_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v21.c", definition)
        self.assertIn("tests/test_policy_cost_v21_recovery.py", definition)
        ...
        self.assertIn("bin/test_policy_cost_v21", versioned_native)
        self.assertNotIn("bin/test_policy_cost\n", versioned_native)
