"""Fail-closed contracts for the fresh-seed policy-cost-v9 recovery."""
...
import yaml
from tools import policy_cost_allocate_v9 as allocator
from tools import policy_cost_calibration_v9 as calibration
from tools import policy_cost_campaign_v9 as campaign


ROOT = Path(__file__).resolve().parents[1]
V8_LAUNCH = "fd50b4101819d8997662dc75b7242a45602c4a4b"
V8_PARENT = "25da27f0f7f70ffdeac0862fd9325e822532148b"
V8_LAUNCH_TREE = "ab7cb5f474e7122f8fdf9f1ac3c7ee4783114750"
V8_PLAN = ROOT / "data/experiments/locked_policy_cost_v8_plan.json"
V9_PLAN = ROOT / "data/experiments/locked_policy_cost_v9_plan.json"
V9_EXECUTION = ROOT / "data/experiments/locked_policy_cost_v9_execution.json"
V9_WORKFLOW = ROOT / ".github/workflows/policy-cost-v9.yml"
V9_DEFINITION = ROOT / ".github/workflows/policy-cost-v9-definition.yml"
V8_FAILURE = ROOT / "data/experiments/policy_cost_v8_run_33257645061_failure.json"
V8_RETAIN = ROOT / "data/experiments/policy_cost_v8_infrastructure_retain.json"
...
class PolicyCostV9RecoveryTests(unittest.TestCase):
    def test_v8_terminal_record_is_complete_and_non_promotable(self) -> None:
        failure, retain = load(V8_FAILURE), load(V8_RETAIN)
        self.assertEqual(failure["schema"],
                         "lc-policy-cost-v8-github-action-resolution-failure-v1")
        ...
        self.assertEqual(failure["github_run"], {
            ...
            "head_sha": V8_LAUNCH, "head_tree": V8_LAUNCH_TREE,
            "id": 33257645061,
            "url": "https://github.com/BEKINDTOEVERYKIND/"
                   "LOSTCITIESCHATGPT/actions/runs/33257645061",
        })
        cause = failure["cause"]
        self.assertEqual(cause["classification"],
                         "github_runner_action_resolution_timeout")
        self.assertFalse(cause["efficacy_related"])
        self.assertEqual(cause["stage"]["job_id"], 99125937958)
        self.assertIn("before checkout", cause["mechanism"])
        self.assertIn("215", cause["mechanism"])
        self.assertEqual(failure["job_disposition"], {
            "failure": 2, "skipped": 10, "success": 223,
            "total": 235, "train_evaluate_success": 215,
        })
        self.assertTrue(
            failure["fixed_seed_execution"]["all_policy_cost_v8_fixed_roots_retired"])
        ...
        self.assertEqual(retain["artifact_count"], 226)
        self.assertEqual(retain["train_evaluation_artifacts"]["count"], 215)
        self.assertEqual(retain["train_evaluation_artifacts"]["missing_indices"], [8])
    ...
    def test_v8_launch_is_the_unique_direct_addendum(self) -> None:
        parent = subprocess.check_output(
            ["git", "rev-parse", f"{V8_LAUNCH}^"], cwd=ROOT, text=True).strip()
        tree = subprocess.check_output(
            ["git", "rev-parse", f"{V8_LAUNCH}^{{tree}}"], cwd=ROOT,
            text=True).strip()
        changed = subprocess.check_output(
            ["git", "diff-tree", "--no-commit-id", "--name-status", "-r", V8_LAUNCH],
            cwd=ROOT, text=True).strip()
        self.assertEqual(parent, V8_PARENT)
        self.assertEqual(tree, V8_LAUNCH_TREE)
        self.assertEqual(changed,
                         "A\tdata/experiments/locked_policy_cost_v8_execution.json")

    def test_every_v8_campaign_byte_is_immutable(self) -> None:
        paths = subprocess.check_output(
            ["git", "ls-tree", "-r", "--name-only", V8_LAUNCH],
        ...
        frozen = [path for path in paths if path.startswith(prefixes) or
                  ...
                  re.fullmatch(r"POLICY_COST_V[1-8]\.md", path)]
        self.assertIn("tools/policy_cost_dataset_v8.c", frozen)
        for relative in frozen:
            current = ROOT / relative
            self.assertTrue(current.is_file(), relative)
            expected = subprocess.check_output(
                ["git", "show", f"{V8_LAUNCH}:{relative}"], cwd=ROOT)
            self.assertEqual(current.read_bytes(), expected, relative)

    def test_all_twenty_one_v9_roots_are_fresh_and_distinct(self) -> None:
        ...
        self.assertTrue(all(root.startswith("202708") for root in roots))
        self.assertFalse(roots & previous)
        burned = plans[-1]["reservoirs"]["burned_source_seeds"]
        self.assertIn("all policy-cost-v8 fixed seeds", burned)
        self.assertIn("every 20270729 feasibility-smoke seed", burned)

    def test_v9_preserves_every_efficacy_design_and_gate(self) -> None:
        v8, v9 = load(V8_PLAN), load(V9_PLAN)
        cal8, cal9 = copy.deepcopy(v8["calibration"]), copy.deepcopy(v9["calibration"])
        cal8["schedule_seed"] = cal9["schedule_seed"]
        cal8["cross_validation"]["seed"] = cal9["cross_validation"]["seed"]
        self.assertEqual(cal8, cal9)
        ctl8, ctl9 = copy.deepcopy(v8["controller"]), copy.deepcopy(v9["controller"])
        ctl8["artifact_binding"]["source_seed"] = ctl9["artifact_binding"]["source_seed"]
        self.assertEqual(ctl8, ctl9)
        self.assertEqual(v8["multiplicity"], v9["multiplicity"])
        self.assertEqual(v8["prerequisite"], v9["prerequisite"])
        self.assertEqual(v8["test"], v9["test"])
        for stage in ("safety", "final"):
            self.assertEqual(without(v8[stage], "seeds"), without(v9[stage], "seeds"))
        self.assertEqual(without(v8["selection"], "bootstrap"),
                         without(v9["selection"], "bootstrap"))
        self.assertEqual(without(v8["selection"]["bootstrap"], "seed"),
                         without(v9["selection"]["bootstrap"], "seed"))
        for key in ("allocation", "candidate_masks",
                    "native_reservoir_origin_proof"):
            self.assertEqual(v8["reservoirs"][key], v9["reservoirs"][key])
        self.assertEqual(seed_agnostic(v8["reservoirs"]["search_and_truth"]),
                         seed_agnostic(v9["reservoirs"]["search_and_truth"]))
        self.assertEqual(
            without(v8["reservoirs"]["discovery"], "seeds", "sealed_smokes"),
            without(v9["reservoirs"]["discovery"], "seeds", "sealed_smokes"))

    def test_batching_reduces_runner_exposure_without_changing_logical_slices(self) -> None:
        workflow = yaml.safe_load(V9_WORKFLOW.read_text(encoding="utf-8"))
        jobs = workflow["jobs"]
    ...
    def test_definition_is_inert_and_launch_is_fail_closed(self) -> None:
        self.assertFalse(V9_EXECUTION.exists())
        workflow = yaml.safe_load(V9_WORKFLOW.read_text(encoding="utf-8"))
        ...
        self.assertIn("tools/policy_cost_dataset_v9.c", text)
        self.assertIn("tools/policy_cost_campaign_v9.py", text)
        self.assertIn("locked_policy_cost_v8_execution.json", text)
        self.assertIn("policy_cost_v8_run_33257645061_failure.json", text)
        self.assertIn("bin/policy_cost_dataset self-test", text)
        definition = V9_DEFINITION.read_text(encoding="utf-8")
        self.assertIn("tools/policy_cost_dataset_v9.c", definition)
        self.assertIn("tests/test_policy_cost_v9_recovery.py", definition)
