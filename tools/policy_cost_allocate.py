...
TRAIN_SCHEMA="LCPOLICYCOST-TRAIN-RESERVOIR-V3"
VECTOR_SCHEMA="LCPOLICYCOST-VECTOR-RESERVOIR-V1"
TRAIN_MANIFEST="LCPOLICYCOST-TRAIN-ALLOCATION-V2"
VECTOR_MANIFEST="LCPOLICYCOST-VECTOR-ALLOCATION-V1"
TRAIN_QUOTA=16; VECTOR_QUOTA=64
TRAIN_CAP=1024; VECTOR_CAP=64
SOURCE_STATE_LIMIT=900
ROUNDS=range(3); PLY_BINS=range(24); RATIO_BINS=range(6); PAIR_TYPES=range(2); SLOTS=range(5)
SEEDS={"TRAIN":202611100101,"SELECT":202611100201,"TEST":202611100301}
...
TRAIN_RULE=(b"lc-policy-cost-train-allocation-v2|canonical-greedy-selection|"
 b"global-source-unique|quota16|rank-major-diagonal-cell-interleave-v1")
VECTOR_RULE=(b"lc-policy-cost-vector-allocation-v1|64-even-feasible-j|"
...
def discovery(path:Path,expected:str)->tuple[dict[str,Any],dict[str,Any],dict[str,Any]]:
 ...
 if len(rows)!=3 or [x.get("record_type") for x in rows] != ["header","census","footer"] or any(x.get("schema")!="lc-policy-cost-discovery-v2" for x in rows): raise AllocationError("discovery schema mismatch")
...
def campaign_discovery_header(split:str,rh:dict[str,Any])->dict[str,Any]:
 return {
  "schema":"lc-policy-cost-discovery-v2","record_type":"header","split":split,
...
def validate_campaign_discovery(dh:dict[str,Any],dc:dict[str,Any],df:dict[str,Any],
 ...
 if set(df)!={"schema","record_type","requested_matches","completed_matches",
    ...
    df.get("schema")!="lc-policy-cost-discovery-v2" or \
 ...
 required_census={"schema","record_type","state_commitment_chain_sha256",
                  ...
                  "allocation_cells"}
 if set(dc)!=required_census or dc.get("schema")!="lc-policy-cost-discovery-v2" or \
 ...
 mask_rows=[uint_vector(row,5,"mask_width_counts") for row in masks]
 unions=uint_vector(dc.get("union_width_counts"),6,"union_width_counts")
 master_widths=uint_vector(dc.get("eligible_master_width_counts"),5,
...
def reservoir(path:Path,expected:str)->tuple[dict[str,Any],list[dict[str,str]],dict[str,Any]]:
 ...
 if split not in SEEDS or head["purpose"]!="campaign" or head["seed"]!=SEEDS[split] or (split=="TRAIN")!=(lines[0]==TRAIN_SCHEMA): raise AllocationError("reservoir campaign identity mismatch")
 if head["cap"]!=(TRAIN_CAP if split=="TRAIN" else VECTOR_CAP): raise AllocationError("reservoir cap changed")
 ...
 for line in lines[8:]:
  ...
  if source>=MATCHES[split]: raise AllocationError("source match is outside frozen discovery")
  ...
  if split=="TRAIN":
   ...
   if fr not in (0,1) or not 1<=width<=5 or sl>=width or cell!=f"r{rd}:p{pb:02d}:f{fr}:j{sl}" or r["master_sha256"]!=r["mask_001_sha256"] or vector_slot(int(head["seed"]),r["state_sha256"],width)!=sl or vector_priority(int(head["seed"]),r)!=r["priority_sha256"] or key in seen_states: raise AllocationError("vector cell/slot/priority mismatch")
   seen_states.add(key)
...
def allocate_vectors(rows:list[dict[str,str]],N:dict[str,int])->tuple[list[dict[str,str]],dict[str,int]]:
 ...
 selected_by_base={}; q={f"r{rd}:p{pb:02d}:f{fr}:j{j}":0 for rd,pb,fr in vector_bases() for j in SLOTS}
 for rd,pb,fr in vector_bases():
  cells=[f"r{rd}:p{pb:02d}:f{fr}:j{j}" for j in SLOTS]; feasible=[c for c in cells if N.get(c,0)>0]
  if not feasible: raise AllocationError(f"empty vector base r{rd}.p{pb}.f{fr}")
  base,rem=divmod(VECTOR_QUOTA,len(feasible)); chosen=[]
  for pos,cell in enumerate(feasible):
   q[cell]=base+(pos<rem)
   if N[cell]<q[cell] or len(by.get(cell,[]))<q[cell]: raise AllocationError(f"sparse positive-q slot {cell}")
   slot_rows=by[cell][:q[cell]]
   if len({x["source_match_index"] for x in slot_rows})<8: raise AllocationError(f"positive-q slot {cell} lacks 8 sources")
   chosen+=slot_rows
...
def build_manifest(dp:Path,dhx:str,rp:Path,rhx:str)->bytes:
 ...
 for x in cells:
  ...
  if split!="TRAIN":
   ...
   if not isinstance(hist,list) or len(hist)!=5 or any(isinstance(v,bool) or not isinstance(v,int) or v<0 for v in hist) or sum(hist)!=N[cell]: raise AllocationError("bad per-cell width histogram")
   widths[cell]=hist
 expected_cells=(set(train_cells()) if split=="TRAIN" else {f"r{rd}:p{pb:02d}:f{fr}:j{j}" for rd,pb,fr in vector_bases() for j in SLOTS})
 ...
 if split=="TRAIN":
  ...
  aggregate=[sum(widths[c][w] for c in widths) for w in range(5)]
  ...
  lines=[VECTOR_MANIFEST,f"split\t{split}","purpose\tcampaign",*common,f"eligible_state_commitment_sha256\t{rf['chain']}",f"allocation_rule_sha256\t{rule}",f"quota_per_base_cell\t{VECTOR_QUOTA}","source_minimum_per_positive_slot\t8",f"total_census\t{total}",f"retained_reservoir_vectors\t{rf['retained']}","poststratum_cells\t720","aggregate_master_width_histogram\t"+",".join(map(str,aggregate)),f"probe_orbit_rejections\t{df['probe_orbit_rejections']}",f"pooled_ge64_observed\t{rf['pooled']}",f"records\t{len(sel)}"]
  for rd,pb,fr in vector_bases():
   for j in SLOTS:
    cell=f"r{rd}:p{pb:02d}:f{fr}:j{j}"; lines.append(f"poststratum\t{cell}\t{N[cell]}\t{q[cell]}\t{N[cell]}\t{total}\t"+",".join(map(str,widths[cell])))
...
def main()->int:
 p=argparse.ArgumentParser(description=__doc__); p.add_argument("--discovery",required=True,type=Path);p.add_argument("--discovery-sha256",required=True);p.add_argument("--reservoir",required=True,type=Path);p.add_argument("--reservoir-sha256",required=True);p.add_argument("--out",required=True,type=Path);a=p.parse_args()
 try: data=build_manifest(a.discovery,a.discovery_sha256,a.reservoir,a.reservoir_sha256);atomic(a.out,data)
