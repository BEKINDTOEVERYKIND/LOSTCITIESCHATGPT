...
SEEDS={"TRAIN":202710100101,"SELECT":202710100201,"TEST":202710100301}
...
def validate_contract_smoke(dp:Path,dhx:str,rp:Path,rhx:str,*,split:str,
 ...
 """Validate a real, small native v11 producer census with no efficacy path.
 ...
 seed_text=str(seed)
 if len(seed_text)!=12 or not seed_text.startswith("20271029"):
  raise AllocationError("contract smoke seed is outside burned 20271029")
 if not 1<=matches<=16:
 ...
 expected_header.update({
  ...
  "seed":seed_text,"seed_domain":"20271029-smoke",
 ...
 payload=(
  b"lc-policy-cost-v11-native-consumer-contract-v1\0"+
