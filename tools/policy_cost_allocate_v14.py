...
SEEDS={"TRAIN":202801100101,"SELECT":202801100201,"TEST":202801100301}
...
def validate_contract_smoke(dp:Path,dhx:str,rp:Path,rhx:str,*,split:str,
 ...
 """Validate a real, small native v14 producer census with no efficacy path.
 ...
 seed_text=str(seed)
 if len(seed_text)!=12 or not seed_text.startswith("20280129"):
  raise AllocationError("contract smoke seed is outside burned 20280129")
 if not 1<=matches<=16:
 ...
 expected_header.update({
  ...
  "seed":seed_text,"seed_domain":"20280129-smoke",
 ...
 payload=(
  b"lc-policy-cost-v14-native-consumer-contract-v1\0"+
