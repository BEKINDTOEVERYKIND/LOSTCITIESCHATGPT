...
TRAIN_MANIFEST="LCPOLICYCOST-TRAIN-ALLOCATION-V3"
...
SEEDS={"TRAIN":202701100101,"SELECT":202701100201,"TEST":202701100301}
...
TRAIN_RULE=(b"lc-policy-cost-train-allocation-v3|canonical-greedy-selection|"
...
def discovery(path:Path,expected:str)->tuple[dict[str,Any],dict[str,Any],dict[str,Any]]:
 ...
 if len(rows)!=3 or [x.get("record_type") for x in rows] != ["header","census","footer"] or any(x.get("schema")!="lc-policy-cost-discovery-v3" for x in rows): raise AllocationError("discovery schema mismatch")
...
def campaign_discovery_header(split:str,rh:dict[str,Any])->dict[str,Any]:
 return {
  "schema":"lc-policy-cost-discovery-v3","record_type":"header","split":split,
...
def validate_campaign_discovery(dh:dict[str,Any],dc:dict[str,Any],df:dict[str,Any],
 ...
 if set(df)!={"schema","record_type","requested_matches","completed_matches",
    ...
    df.get("schema")!="lc-policy-cost-discovery-v3" or \
 ...
 required_census={"schema","record_type","state_commitment_chain_sha256",
                  ...
                  "allocation_cells"}
 if set(dc)!=required_census or dc.get("schema")!="lc-policy-cost-discovery-v3" or \
