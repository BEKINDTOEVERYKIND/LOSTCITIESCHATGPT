...
TRAIN_SCHEMA="LCPOLICYCOST-TRAIN-RESERVOIR-V4"
VECTOR_SCHEMA="LCPOLICYCOST-VECTOR-RESERVOIR-V1"
TRAIN_MANIFEST="LCPOLICYCOST-TRAIN-ALLOCATION-V4"
...
ROUNDS=range(3); PLY_BINS=range(24); RATIO_BINS=range(6); PAIR_TYPES=range(2); SLOTS=range(MASK_MAX)
SEEDS={"TRAIN":202702100101,"SELECT":202702100201,"TEST":202702100301}
...
TRAIN_RULE=(b"lc-policy-cost-train-allocation-v4|canonical-greedy-selection|"
...
def discovery(path:Path,expected:str)->tuple[dict[str,Any],dict[str,Any],dict[str,Any]]:
 ...
 if len(rows)!=3 or [x.get("record_type") for x in rows] != ["header","census","footer"] or any(x.get("schema")!="lc-policy-cost-discovery-v4" for x in rows): raise AllocationError("discovery schema mismatch")
...
def campaign_discovery_header(split:str,rh:dict[str,Any])->dict[str,Any]:
 return {
  "schema":"lc-policy-cost-discovery-v4","record_type":"header","split":split,
  ...
  "burned_source_deal_seeds":(
   ...
   "feasibility-smoke seed, 202701010101, 202702010101, and every "
   "20270229 feasibility-smoke seed"
  ),
 }
...
def validate_campaign_discovery(dh:dict[str,Any],dc:dict[str,Any],df:dict[str,Any],
 ...
 if set(df)!={"schema","record_type","requested_matches","completed_matches",
    ...
    df.get("schema")!="lc-policy-cost-discovery-v4" or \
 ...
 required_census={"schema","record_type","state_commitment_chain_sha256",
                  ...
                  "allocation_cells"}
 if set(dc)!=required_census or dc.get("schema")!="lc-policy-cost-discovery-v4" or \
...
def validate_contract_smoke(dp:Path,dhx:str,rp:Path,rhx:str,*,split:str,
 ...
 """Validate a real, small native v4 producer census with no efficacy path.
 ...
 seed_text=str(seed)
 if len(seed_text)!=12 or not seed_text.startswith("20270229"):
  raise AllocationError("contract smoke seed is outside burned 20270229")
 if not 1<=matches<=16:
 ...
 expected_header.update({
  ...
  "seed":seed_text,"seed_domain":"20270229-smoke",
 ...
 payload=(
  b"lc-policy-cost-v4-native-consumer-contract-v1\0"+
