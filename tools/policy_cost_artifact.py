...
def read_policy_cost(path: Path) -> dict[str, Any]:
    ...
    if words[7] != 1 or words[8] != 1:
        raise ArtifactError(
            "policy-cost v1 requires exact-terminal uniform-belief mode"
        )
    ...
    if words[14] not in {0, 4, 8, 10, 12, 14} or words[15] != 0:
        raise ArtifactError("invalid policy-cost ply window")
    ...
    lambda_action: list[float] = []
    lambda_draw: list[float] = []
    for index in range(len(ANCHORS)):
        action = _f64(raw, payload_offset + 16 * index)
        draw = _f64(raw, payload_offset + 16 * index + 8)
        if not (
            math.isfinite(action)
            and math.isfinite(draw)
            and 0 <= action <= 1000
            and 0 <= draw <= 1000
        ):
            raise ArtifactError("invalid policy-cost spline coefficient")
        lambda_action.append(action)
        lambda_draw.append(draw)

    return {
        "schema": "lc-policy-cost-v1",
        "path": str(path),
