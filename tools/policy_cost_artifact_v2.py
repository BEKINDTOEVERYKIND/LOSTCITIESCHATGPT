...
def read_policy_cost(path: Path) -> dict[str, Any]:
    ...
    if words[7] != 1 or words[8] != 1:
        raise ArtifactError(
            "policy-cost v2 requires exact-terminal uniform-belief mode"
        )
    ...
    if words[14] != 0 or words[15] != 0:
        raise ArtifactError("policy-cost v2 requires all-ply search")
