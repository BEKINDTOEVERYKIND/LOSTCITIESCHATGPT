#!/usr/bin/env python3
"""Deterministic calibration for a policy-prior search cost.

The deployable rule produced here is deliberately a scalar potential::

    score(a) = search_q(a)
             + lambda_core(ply) * log(policy_core_probability(a))
             + lambda_draw(ply) * log(policy_draw_given_core(a))
...
Training observations compare a search-panel action-value difference with an
*independent* truth-panel difference.  Their residual and design row are
standardized by ``sqrt(search_se**2 + truth_se**2 + se_floor**2)`` before
Huber fitting.
...
DEFAULT_FOLD_SEED = "202611140101"
MAX_ROUND_PLY = 299
...
class PairObservation:
    ...
    fitted schedule predicts ``truth_delta`` as ``search_delta`` plus its two
    log-prior corrections.  Reversing a pair negates all four quantities and
...
class FitConfig:
    ...
    min_core_lambda: float = 0.0
    min_draw_lambda: float = 0.0
    ...
    def validated(self) -> "FitConfig":
        ...
        core_min = _finite(self.min_core_lambda, "min_core_lambda")
        draw_min = _finite(self.min_draw_lambda, "min_draw_lambda")
        if core_min < 0.0 or draw_min < 0.0:
            raise CalibrationError("lambda lower bounds must be non-negative")
        ...
        return FitConfig(
            ...
            fold_seed=seed,
            huber_delta=delta,
            min_core_lambda=core_min,
            min_draw_lambda=draw_min,
...
class PolicyCostSchedule:
    anchors: tuple[int, ...]
    lambda_core: tuple[float, ...]
    lambda_draw: tuple[float, ...]

    def validated(self) -> "PolicyCostSchedule":
        anchors = _validate_anchors(self.anchors)
        core = tuple(_finite(value, "lambda_core")
                     for value in self.lambda_core)
        draw = tuple(_finite(value, "lambda_draw")
                     for value in self.lambda_draw)
        if len(core) != len(anchors) or len(draw) != len(anchors):
            raise CalibrationError("each lambda schedule needs one value per anchor")
        if any(value < 0.0 for value in core + draw):
            raise CalibrationError("policy-cost lambdas must be non-negative")
        return PolicyCostSchedule(anchors, core, draw)

    def lambdas_at(self, ply: int) -> tuple[float, float]:
        checked = self.validated()
        basis = linear_spline_basis(ply, checked.anchors)
        return (
            float(basis @ np.asarray(checked.lambda_core)),
            float(basis @ np.asarray(checked.lambda_draw)),
        )
    ...
    def score(
        ...
        *,
        ...
        return q + lambda_core * math.log(core) + lambda_draw * math.log(draw)
...
class CalibrationResult:
    ...
    min_core_lambda: float
    min_draw_lambda: float
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        value: dict[str, Any] = {
            ...
            "model": {
                "score": (
                    "Q + lambda_core(ply)*log(Pcore) + "
                    "lambda_draw(ply)*log(Pdraw_given_core)"
                ),
            ...
            "schedule": {
                ...
                "lambda_core": list(schedule.lambda_core),
                "lambda_draw": list(schedule.lambda_draw),
            },
            "fit": {
                ...
                "row_weighting": (
                    ...
                    "search-panel row in that state"
                ),
                ...
                "min_core_lambda": self.min_core_lambda,
                "min_draw_lambda": self.min_draw_lambda,
                "solver": {
            ...
            "model_adequacy": dict(self.model_adequacy),
            ...
            "derived_gap_thresholds": derived_gap_threshold_table(schedule),
        }
...
def _design_rank_diagnostics(
    ...
    expected_rank = 2 * len(config.anchors)
...
def _design(
    ...
    design = np.zeros((len(observations), 2 * knot_count), dtype=np.float64)
    ...
    for index, item in enumerate(observations):
        ...
        target[index] = (
            item.truth_delta - item.search_delta
        ) / variance_scale
        weights[index] = item.state_weight / state_rows[item.state_id]
    return design, target, weights


def _roughness_matrix(anchors: Sequence[int]) -> np.ndarray:
    ...
    result = np.zeros((2 * knot_count, 2 * knot_count), dtype=np.float64)
    result[:knot_count, :knot_count] = one
    result[knot_count:, knot_count:] = one
    return result
...
def _fit_one(
    ...
    *,
    ...
    lower = np.concatenate((
        np.full(knot_count, config.min_core_lambda, dtype=np.float64),
        np.full(knot_count, config.min_draw_lambda, dtype=np.float64),
    ))
    shifted_target = target - design @ lower
    roughness = _roughness_matrix(anchors)
...
def _fit_nonnegative_matrix(
    ...
    for iteration in range(config.max_irls_iterations):
        ...
        linear = design.T @ (effective * target)
        updated = _solve_nonnegative_quadratic(
            hessian,
            linear,
            result,
        ...
        losses = _huber_values(target - design @ updated, config.huber_delta)
        objective = float(np.sum(sample_weights * losses) / weight_sum)
        change = float(np.max(np.abs(updated - result)))
        objective_change = abs(previous_objective - objective)
        result = updated
...
def _standardized_pair_design(
    ...
    design = np.empty((len(observations), 2), dtype=np.float64)
    target = np.empty(len(observations), dtype=np.float64)
    weights = np.empty(len(observations), dtype=np.float64)
    state_rows: dict[str, int] = {}
    for item in observations:
    ...
    for index, item in enumerate(observations):
        ...
        target[index] = (item.truth_delta - item.search_delta) / scale
        weights[index] = item.state_weight / state_rows[item.state_id]
    return design, target, weights
...
def _cell_design_diagnostic(
    ...
    return {
        ...
        "active_columns": [
            "log_core_ratio" if index == 0 else "log_draw_ratio"
            for index in active
        ],
        "expected_rank": expected_rank,
        "rank_after_column_scaling": rank,
...
def _predict_partition(
    ...
    if model == "zero_policy_cost":
        return np.zeros(len(validation), dtype=np.float64)
    if model == "shared_log_linear":
    ...
    if model == "round_specific_log_linear":
        ...
        return predictions
    if model == "cell_saturated_log_linear":
        predictions = np.full(len(validation), np.nan, dtype=np.float64)
        train_by_cell: dict[
...
def _one_se_smoothness(
    ...
    *,
    ...
    if model not in ("shared_log_linear", "round_specific_log_linear"):
...
def _nested_adequacy_predictions(
    ...
    for outer_fold in range(config.folds):
        ...
        for model in ("shared_log_linear", "round_specific_log_linear"):
            inner_seed = (
...
def _model_adequacy(
    ...
    baseline = means["shared_log_linear"]
    ...
    zero_loss = np.asarray(losses["zero_policy_cost"], dtype=np.float64)
    shared_loss = np.asarray(losses["shared_log_linear"], dtype=np.float64)
    paired_gain = zero_loss - shared_loss
    ...
    row_gain = (
        row_losses["zero_policy_cost"] - row_losses["shared_log_linear"]
    )
    ...
    return {
        ...
        "hyperparameter_tuning": (
            "all shared and round-specific smoothness choices are made by "
            "inner source-match-grouped CV inside each outer training fold"
        ),
        "nested_outer_folds": nested_selections,
        "shared_model": "round-shared ply spline x log policy factors",
        "challengers": [
            "round-specific ply splines",
            "round x ply-bin x ratio-bin x pair-type saturated slopes",
        ],
        ...
        "relative_improvement_over_shared": improvements,
        "maximum_allowed_relative_improvement": (
            config.model_lack_max_relative_improvement
        ),
        "rule": (
            "report whether the one-sided OOF loss-reduction LCB versus zero "
            "cost is strictly positive and whether either richer challenger "
            "reduces OOF Huber loss by more than the frozen relative limit; "
            "this preregistered model-lack result is diagnostic only"
        ),
...
def calibrate_policy_cost(
    ...
    if campaign_evidence_binding is None:
        ...
        if not checked_config.require_campaign_design or set(campaign_evidence_binding) != required_evidence or \
                campaign_evidence_binding.get("schema") != "lc-policy-cost-v1-train-evidence-binding-v1" or \
    ...
    schedule = PolicyCostSchedule(
        anchors=checked_config.anchors,
        lambda_core=tuple(float(value) for value in coefficients[:knot_count]),
        lambda_draw=tuple(float(value) for value in coefficients[knot_count:]),
    ).validated()
    ...
    return CalibrationResult(
        ...
        min_core_lambda=checked_config.min_core_lambda,
        min_draw_lambda=checked_config.min_draw_lambda,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--min-core-lambda", type=float, default=0.0)
    parser.add_argument("--min-draw-lambda", type=float, default=0.0)
    parser.add_argument(
    ...
    config = FitConfig(
        ...
        min_core_lambda=arguments.min_core_lambda,
        min_draw_lambda=arguments.min_draw_lambda,
