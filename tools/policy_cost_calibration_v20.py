...
The deployable draw-source rule is deliberately a scalar potential::

    score(a) = beta_search(ply) * search_q(a)
             + alpha_draw(ply) * log(policy_draw_given_core(a))

The action-core coefficient is fixed exactly zero.  The runtime first chooses
the semantic action core by raw search Q, then applies this score only among
draw sources for that same action.  The draw term is also structurally zero
for 16 <= ply < 40.  Runtime confidence and minimum-advantage guards remain
separate concerns.

Training observations predict an *independent* truth-panel action-value
difference from the search-panel difference and the conditional-draw gap on
same-action rows.  Search
...
# v16's completed nested TRAIN diagnostic found a small negative incremental
# benefit from the policy-prior terms in the frozen midgame stratum while the
# early and late strata remained positive.  v20 therefore tests one new,
# outcome-blind deployment family on fresh roots: search shrinkage remains a
# full spline, but both policy costs are structurally zero for 16 <= ply < 40.
# The boundary knots at 16, 24 and 32 are fixed zero rather than estimated.
POLICY_ZERO_PHASE = (16, 40)
DEFAULT_STANDARD_ERROR_FLOOR = 0.25
DEFAULT_MIN_SEARCH_BETA = 1.0e-6
DEFAULT_FOLD_SEED = "202807140101"
MAX_ROUND_PLY = 299
...
def policy_active_anchor_indices(anchors: Sequence[int]) -> tuple[int, ...]:
    """Indices of policy-cost knots that v20 is allowed to estimate."""

    checked = _validate_anchors(anchors)
    lower, upper = POLICY_ZERO_PHASE
    if lower not in checked or upper not in checked:
        raise CalibrationError(
            "phase-selective policy costs require boundary anchors 16 and 40"
        )
    return tuple(
        index for index, anchor in enumerate(checked)
        if anchor < lower or anchor >= upper
    )


def phase_selective_policy_basis(
    ply: int, anchors: Sequence[int],
) -> np.ndarray:
    """Policy-cost basis with an exact zero midgame interval.

    Early coefficients interpolate on the ordinary full spline toward a
    fixed-zero boundary at ply 16.  Plies 16..39 are exactly zero.  Late
    coefficients start independently at ply 40 and interpolate thereafter.
    The returned vector contains only estimable knots, in full-anchor order.
    """

    checked = _validate_anchors(anchors)
    active = policy_active_anchor_indices(checked)
    if isinstance(ply, bool) or int(ply) != ply:
        raise CalibrationError("ply must be an integer")
    value = int(ply)
    if not checked[0] <= value <= MAX_ROUND_PLY:
        raise CalibrationError("ply lies outside the frozen round range")
    lower, upper = POLICY_ZERO_PHASE
    full = linear_spline_basis(value, checked)
    if lower <= value < upper:
        return np.zeros(len(active), dtype=np.float64)
    return full[np.asarray(active, dtype=np.int64)]


def expand_phase_selective_policy_coefficients(
    values: Sequence[float], anchors: Sequence[int],
) -> tuple[float, ...]:
    """Expand estimable policy knots to the runtime's full anchor vector."""

    checked = _validate_anchors(anchors)
    active = policy_active_anchor_indices(checked)
    supplied = tuple(_finite(value, "phase-selective policy coefficient")
                     for value in values)
    if len(supplied) != len(active):
        raise CalibrationError("phase-selective policy coefficient width differs")
    result = [0.0] * len(checked)
    for index, value in zip(active, supplied):
        if value < 0.0:
            raise CalibrationError("policy coefficients must be non-negative")
        result[index] = value
    return tuple(result)
...
def _design_rank_diagnostics(
    ...
    expected_rank = (
        len(config.anchors)
        + len(policy_active_anchor_indices(config.anchors))
    )
...
def _full_design(
    ...
    """Unrestricted three-spline design retained as a richer challenger."""
...
def _design(
    ...
    """Hierarchical draw-gap design with core and midgame costs fixed zero."""

    knot_count = len(anchors)
    policy_count = len(policy_active_anchor_indices(anchors))
    design = np.zeros(
        (len(observations), knot_count + policy_count), dtype=np.float64
    )
    target = np.empty(len(observations), dtype=np.float64)
    weights = np.empty(len(observations), dtype=np.float64)
    state_rows: dict[str, int] = {}
    for item in observations:
        state_rows[item.state_id] = state_rows.get(item.state_id, 0) + 1
    for index, item in enumerate(observations):
        search_basis = linear_spline_basis(item.ply, anchors)
        policy_basis = phase_selective_policy_basis(item.ply, anchors)
        variance_scale = math.hypot(
            item.search_se, item.truth_se, standard_error_floor
        )
        design[index, :knot_count] = (
            search_basis * item.search_delta / variance_scale
        )
        design[index, knot_count:] = (
            policy_basis * (
                item.log_draw_ratio
                if item.pair_type == "same_core_draw" else 0.0
            ) / variance_scale
        )
        target[index] = item.truth_delta / variance_scale
        weights[index] = item.state_weight / state_rows[item.state_id]
...
def _phase_selective_roughness_matrix(anchors: Sequence[int]) -> np.ndarray:
    """Block roughness for beta plus disconnected early/late policy splines."""

    checked = _validate_anchors(anchors)
    active = policy_active_anchor_indices(checked)
    knot_count = len(checked)
    policy_count = len(active)
    result = np.zeros(
        (knot_count + 2 * policy_count,) * 2, dtype=np.float64
    )
    result[:knot_count, :knot_count] = _roughness_matrix(checked, 1)
    lower, upper = POLICY_ZERO_PHASE
    segments = [
        tuple(index for index in active if checked[index] < lower),
        tuple(index for index in active if checked[index] >= upper),
    ]
    for block in range(2):
        offset = knot_count + block * policy_count
        for segment in segments:
            if len(segment) < 3:
                continue
            local_positions = tuple(active.index(index) for index in segment)
            local_anchors = tuple(checked[index] for index in segment)
            local = _roughness_matrix(local_anchors, 1)
            for row, left in enumerate(local_positions):
                for column, right in enumerate(local_positions):
                    result[offset + left, offset + right] = local[row, column]
    return result


def _phase_selective_draw_roughness_matrix(
    anchors: Sequence[int],
) -> np.ndarray:
    """Roughness for beta plus one disconnected conditional-draw spline."""

    checked = _validate_anchors(anchors)
    knot_count = len(checked)
    policy_count = len(policy_active_anchor_indices(checked))
    full = _phase_selective_roughness_matrix(checked)
    selected = tuple(range(knot_count)) + tuple(
        range(knot_count + policy_count, knot_count + 2 * policy_count)
    )
    return full[np.ix_(selected, selected)]
...
def _fit_one(
    ...
    *,
    ...
    if design.shape[1] == knot_count:
    ...
    elif design.shape[1] == knot_count + policy_count:
        coefficient_blocks = 2
        lower = np.concatenate((
            np.full(knot_count, config.min_search_beta, dtype=np.float64),
            np.full(policy_count, config.min_draw_alpha, dtype=np.float64),
        ))
        roughness = _phase_selective_draw_roughness_matrix(anchors)
        ...
        roughness = _roughness_matrix(anchors, coefficient_blocks)
    elif design.shape[1] == knot_count + 2 * policy_count:
        coefficient_blocks = 3
        lower = np.concatenate((
            np.full(knot_count, config.min_search_beta, dtype=np.float64),
            np.full(policy_count, config.min_core_alpha, dtype=np.float64),
            np.full(policy_count, config.min_draw_alpha, dtype=np.float64),
        ))
        roughness = _phase_selective_roughness_matrix(anchors)
    else:
...
def _fit_nonnegative_matrix(
    ...
    final_objective_change = math.inf
    iterations = 0
    diagnostic_tail: list[tuple[int, float, float, float]] = []
    for iteration in range(config.max_irls_iterations):
        ...
        final_change = change
        final_objective_change = objective_change
        diagnostic_tail.append((iterations, change, objective_change, objective))
        ...
        residual = shifted_target - design @ result
        score = np.clip(
            residual, -config.huber_delta, config.huber_delta
        )
        gradient = -(design.T @ (sample_weights * score)) / weight_sum
        projected = np.where(result > 0.0, gradient, np.minimum(gradient, 0.0))
        raise CalibrationError(
            "model-adequacy IRLS did not converge "
            f"(fit={fit_label!r}, rows={design.shape[0]}, "
            f"columns={design.shape[1]}, final_change={final_change:.17g}, "
            "final_objective_change="
            f"{final_objective_change:.17g}, "
            f"projected_gradient_max={float(np.max(np.abs(projected))):.17g}, "
            f"coefficients={result.tolist()!r}, tail={diagnostic_tail!r})"
        )
    result = result + lower
...
def _cell_supported_columns(
    cell: tuple[int, int, int, str], design: np.ndarray,
) -> tuple[tuple[int, ...], np.ndarray, tuple[int, ...]]:
    """Return the estimable columns of one cell-saturated fit.

    ``different_core`` allocations orient the semantic-core ratio, but they
    do not require the two complete moves to use different draw sources.  In
    that valid same-draw subcase the conditional-draw ratio is identically
    zero throughout a cell (or an outer-fold training partition), so there is
    no draw coefficient to estimate.  Treating that structural zero as a
    rank failure rejects a valid design before the adequacy comparison.

    Search is always required.  The defining ratio for each pair type is also
    required: draw for ``same_core_draw`` and core for ``different_core``.
    Only the optional draw column of a ``different_core`` cell may be omitted,
    and the omission is derived solely from the training design matrix.
    """

    candidates = _cell_active_columns(cell[3])
    candidate_norms = np.linalg.norm(design[:, candidates], axis=0)
    if np.any(~np.isfinite(candidate_norms)):
        raise CalibrationError(
            f"cell-saturated adequacy cell {cell!r} has non-finite column norm"
        )
    active = tuple(
        column for column, norm in zip(candidates, candidate_norms)
        if norm > 0.0
    )
    inactive = tuple(column for column in candidates if column not in active)
    required = (0, 2) if cell[3] == "same_core_draw" else (0, 1)
    missing_required = tuple(column for column in required if column not in active)
    if missing_required:
        raise CalibrationError(
            f"cell-saturated adequacy cell {cell!r} has zero required "
            f"columns {missing_required!r}"
        )
    return active, candidate_norms, inactive
...
def _cell_design_diagnostic(
    ...
    active, candidate_norms, inactive = _cell_supported_columns(cell, design)
    ...
    return {
        ...
        "inactive_structural_zero_columns": [
            ("search_delta" if index == 0 else
             "log_core_ratio" if index == 1 else "log_draw_ratio")
            for index in inactive
        ],
        "candidate_column_norms": {
            ("search_delta" if index == 0 else
             "log_core_ratio" if index == 1 else "log_draw_ratio"): float(norm)
            for index, norm in zip(_cell_active_columns(cell[3]), candidate_norms)
        },
        "expected_rank": expected_rank,
        "rank_after_column_scaling": rank,
        "condition_number_after_column_scaling": condition,
...
def _cell_saturated_identifiability(
    ...
    return {
        ...
        "supported_rank_contract_by_pair_type": {
            "different_core": (
                "search and semantic-core columns required; conditional-draw "
                "column included exactly when nonzero in the training partition"
            ),
            "same_core_draw": "search and conditional-draw columns required",
        },
...
def _predict_partition(
    ...
    if model == "draw_only_gap":
        ...
        coefficient = _fit_one(
            ...
            config=config,
            convergence_log=convergence_log,
            fit_label=f"{fit_context}:draw-only-gap",
        )
        ...
        return validation_x @ coefficient
    if model == "full_gap":
        train_x, train_y, train_w = _full_design(
            training, config.anchors, config.standard_error_floor
        )
        coefficient = _fit_one(
            train_x, train_y, train_w,
            ...
            config=config,
            convergence_log=convergence_log,
            fit_label=f"{fit_context}:full-gap",
        )
        validation_x, _, _ = _full_design(
            validation, config.anchors, config.standard_error_floor
        )
        return validation_x @ coefficient
    ...
    if model == "cell_saturated":
        ...
        for cell in sorted(validation_by_cell):
            ...
            diagnostic = _cell_design_diagnostic(cell, cell_training, config)
            names = ("search_delta", "log_core_ratio", "log_draw_ratio")
            active = tuple(
                names.index(name) for name in diagnostic["active_columns"]
            )
...
def _one_se_smoothness(
    ...
    *,
    ...
    if model not in (
        "beta_only", "draw_only_gap", "full_gap", "round_specific",
    ):
...
def _nested_adequacy_predictions(
    ...
    # The deployed hierarchy never compares a conditional-draw adjustment
    # across semantic action cores, and its policy term is exactly zero in the
    # protected midgame.  Preserve the independently cross-fitted beta-only
    # prediction on those decision-invariant rows so the adequacy test grades
    # the model actually deployed rather than a coupled regression artifact.
    inactive = np.asarray([
        item.pair_type != "same_core_draw" or 16 <= item.ply < 40
        for item in observations
    ], dtype=bool)
    predictions["draw_only_gap"][inactive] = predictions["beta_only"][inactive]
    predictions["round_specific"][inactive] = predictions["beta_only"][inactive]
...
def _model_adequacy(
    ...
    baseline = means["draw_only_gap"]
    improvements: dict[str, float] = {}
    for model in ("full_gap", "round_specific", "cell_saturated"):
        improvements[model] = (
            0.0 if baseline == 0.0
            else (baseline - means[model]) / baseline
        )
    beta_loss = np.asarray(losses["beta_only"], dtype=np.float64)
    deployable_loss = np.asarray(
        losses["draw_only_gap"], dtype=np.float64
    )
    ...
    row_gain = (
        row_losses["beta_only"] - row_losses["draw_only_gap"]
    )
    ...
    return {
        ...
        "deployable_model": (
            "full round-shared beta_search and conditional-draw alpha_draw "
            "splines, with alpha_core fixed exactly zero at every ply"
        ),
        "comparators": [
            "identity search T=S",
        ...
        "challengers": [
            "unrestricted round-shared beta/alpha ply splines",
            "round-specific phase-selective beta/alpha ply splines",
...
def calibrate_policy_cost(
    ...
    if campaign_evidence_binding is None:
        ...
        if not checked_config.require_campaign_design or set(campaign_evidence_binding) != required_evidence or \
                campaign_evidence_binding.get("schema") != "lc-policy-cost-v20-train-evidence-binding-v1" or \
    ...
    policy_count = len(policy_active_anchor_indices(checked_config.anchors))
    schedule = PolicyCostSchedule(
        anchors=checked_config.anchors,
        beta_search=tuple(
            float(value) for value in coefficients[:knot_count]
        ),
        alpha_core=tuple(0.0 for _ in checked_config.anchors),
        alpha_draw=expand_phase_selective_policy_coefficients(
            coefficients[knot_count:knot_count + policy_count],
            checked_config.anchors,
        ),
    ).validated()
