...
_Static_assert(PC_MASK_MAX == 5 && PC_UNION_MAX == 5 &&
               PC_VECTOR_GROUPS == 3,
               "runtime-mask/union census contract is exactly five bins");
...
static const SplitDomain SPLIT_DOMAIN[3] = {
    { "TRAIN",  UINT64_C(202807100101), UINT64_C(202807110101),
      UINT64_C(202807120101), UINT64_C(202807130101),
      UINT64_C(202807160101),
      PC_TRAIN_TRUTH_WORLDS },
    { "SELECT", UINT64_C(202807100201), UINT64_C(202807110201),
      UINT64_C(202807120201), UINT64_C(202807130201),
      UINT64_C(202807160201),
      PC_HOLDOUT_TRUTH_WORLDS },
    { "TEST",   UINT64_C(202807100301), UINT64_C(202807110301),
      UINT64_C(202807120301), UINT64_C(202807130301),
      UINT64_C(202807160301),
      PC_HOLDOUT_TRUTH_WORLDS },
};
                           ...
                           int union_index[PC_UNION_MAX],int *nunion)
{
    ...
    *nunion=mask_union(mask,union_index);
    /* mask[1] is a subsequence of mask[0], so their union is exactly the
     * one-percent master.  The allocator independently checks the resulting
     * width histograms before it can allocate any campaign evidence. */
    return *nunion==mask[0].n&&*nunion>=1&&*nunion<=PC_UNION_MAX;
}
...
typedef struct {
    ...
    /* Candidate slots two through four are a single outcome-blind tail
     * poststratum.  Every retained row still records its exact slot and its
     * complete max-five vector; only the sampling cell is coarsened. */
    VectorReservoir vector[3][PC_PLY_BINS][2][PC_VECTOR_GROUPS];
...
static int vector_post_group(int slot)
{
                             ...
                             size_t state_n,const unsigned char state_hash[32])
{
    ...
    int group=vector_post_group(slot);
    VectorReservoir *cell=&c->vector[view->round][pb][frontier][group];
...
static int census_init(DiscoveryCensus *c,int capacity,uint64_t seed)
{
    ...
    for(int r=0;r<3;r++)for(int p=0;p<PC_PLY_BINS;p++)for(int f=0;f<2;f++)
      for(int g=0;g<PC_VECTOR_GROUPS;g++){
        VectorReservoir *cell=&c->vector[r][p][f][g];cell->cap=capacity;
...
static void census_free(DiscoveryCensus *c)
{
    ...
    for(int r=0;r<3;r++)for(int p=0;p<PC_PLY_BINS;p++)for(int f=0;f<2;f++)
      for(int g=0;g<PC_VECTOR_GROUPS;g++)free(c->vector[r][p][f][g].entry);
    memset(c,0,sizeof *c);
}
...
static int smoke_seed_valid(uint64_t seed)
{
    ...
    return strlen(b)==12&&!strncmp(b,"20280729",8);
}
...
static int run_discover(const DiscoverConfig *c)
{
    ...
    fprintf(evidence.file,"{\"schema\":\"lc-policy-cost-discovery-v5\","
        ...
        (unsigned long long)root_seed,c->smoke?"20280729-smoke":"locked-discovery",
    ...
    for(int w=1;w<=PC_MASK_MAX;w++){
      ...
      for(int rd=0;rd<3;rd++)for(int pb=0;pb<PC_PLY_BINS;pb++)
       for(int fr=0;fr<2;fr++)for(int g=0;g<PC_VECTOR_GROUPS;g++)
        total_width+=census.vector[rd][pb][fr][g].width_count[w];
    ...
    if(c->split==SPLIT_TRAIN){
      ...
      for(int rd=0;rd<3;rd++)for(int pb=0;pb<PC_PLY_BINS;pb++)
       for(int fr=0;fr<2;fr++)for(int g=0;g<PC_VECTOR_GROUPS;g++){
        VectorReservoir *cell=&census.vector[rd][pb][fr][g];
        fprintf(evidence.file,"%s{\"cell\":\"r%d:p%02d:f%d:g%d\","
            "\"eligible_vectors\":%llu,\"retained_vectors\":%d,"
            "\"master_width_histogram\":[%llu,%llu,%llu,%llu,%llu]}",
            first_cell?"":",",rd,pb,fr,g,(unsigned long long)cell->eligible,cell->n,
      ...
      for(int rd=0;rd<3;rd++)for(int pb=0;pb<PC_PLY_BINS;pb++)
       for(int fr=0;fr<2;fr++)for(int g=0;g<PC_VECTOR_GROUPS;g++){
        ...
        for(int i=0;i<cell->n;i++){
            ...
            source_id(c->split,e->source_match_index,sid);
            fprintf(reservoir.file,"r%d:p%02d:f%d:g%d\t%s\t%llu\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t"
                "%s\t%s\t%s\t%s\t%s\t%s\n",rd,pb,fr,g,pri,
                (unsigned long long)e->source_match_index,e->source_state_index,sid,
                rd,pb,fr,e->allocation_slot,e->master_width,oh,shx,m0,m1,mh,state_hex);
            retained++;
        }
      }
    }
...
typedef struct {
    ...
    uint64_t post_n[3][PC_PLY_BINS][2][PC_VECTOR_GROUPS];
    int post_q[3][PC_PLY_BINS][2][PC_VECTOR_GROUPS];
    Allocation *row;
} AllocationManifest;
...
static void expected_allocation_rule(int is_vector,unsigned char out[32])
{
    static const char train[]=
        "lc-policy-cost-train-allocation-v5|canonical-greedy-selection|"
        "global-source-unique|quota16|rank-major-diagonal-cell-interleave-v1";
    static const char vector[]=
        "lc-policy-cost-vector-allocation-v2|64-fixed-g0-g1-tail|priority-v1|"
                           ...
                           AllocationManifest *m,unsigned char file_hash[32])
{
    ...
    if(!strcmp(line,"LCPOLICYCOST-TRAIN-ALLOCATION-V5\n"))m->is_vector=0;
    else if(!strcmp(line,"LCPOLICYCOST-VECTOR-ALLOCATION-V2\n"))m->is_vector=1;
    else goto bad;
    ...
    if(!m->is_vector){
      ...
      if(!header_value(f,&line,&cap,"eligible_state_commitment_sha256",&v)||
         ...
         !header_value(f,&line,&cap,"source_minimum_per_poststratum",&v)||
         ...
         !parse_int(v,432,432,&post_cells)||
      ...
      for(int rd=0;rd<3;rd++)for(int pb=0;pb<PC_PLY_BINS;pb++)
       for(int fr=0;fr<2;fr++)for(int group=0;group<PC_VECTOR_GROUPS;group++){
        ...
        for(int w=0;w<PC_MASK_MAX;w++){width_sum+=wc[w];seen_width[w]+=wc[w];
          /* g0 and g1 cannot contain a master width below g+1.  The tail
           * group contains exact slots 2..4 and therefore requires K>=3. */
          if(w<group&&wc[w])goto bad;}
        char id[64];snprintf(id,sizeof id,"r%d:p%02d:f%d:g%d",rd,pb,fr,group);
        if(strcmp(id,x[1])||nn<(uint64_t)qq||width_sum!=nn)goto bad;
        m->post_n[rd][pb][fr][group]=nn;m->post_q[rd][pb][fr][group]=qq;
        sum_n+=nn;sum_q+=qq;
      }
    ...
    int vector_counts[3][PC_PLY_BINS][2][PC_VECTOR_GROUPS]={0};
    int train_previous[3][PC_PLY_BINS][PC_RATIO_BINS][PC_PAIR_TYPES];
    int vector_previous[3][PC_PLY_BINS][2][PC_VECTOR_GROUPS];
    for(int rd=0;rd<3;rd++)for(int pb=0;pb<PC_PLY_BINS;pb++){
      ...
      for(int frontier=0;frontier<2;frontier++)
       for(int group=0;group<PC_VECTOR_GROUPS;group++)
        vector_previous[rd][pb][frontier][group]=-1;
    }
    ...
    for(int r=0;r<m->records;r++){
        ...
        if(!m->is_vector){
          ...
          int group=vector_post_group(a->allocation_slot);
          char id[64];snprintf(id,sizeof id,"r%d:p%02d:f%d:g%d",a->round,a->ply_bin,
                               a->frontier,group);
          if(strcmp(id,x[9])||census!=m->post_n[a->round][a->ply_bin][a->frontier][group]||
             quota!=(uint64_t)m->post_q[a->round][a->ply_bin][a->frontier][group]||
          ...
          int prior=vector_previous[a->round][a->ply_bin][a->frontier][group];
          if(prior>=0&&!allocation_tuple_before(&m->row[prior],a))goto bad;
          vector_previous[a->round][a->ply_bin][a->frontier][group]=r;
          vector_counts[a->round][a->ply_bin][a->frontier][group]++;
        }
    ...
    if(!m->is_vector){
      ...
      static const int required_q[PC_VECTOR_GROUPS]={22,21,21};
      for(int rd=0;rd<3;rd++)for(int p=0;p<PC_PLY_BINS;p++)for(int fr=0;fr<2;fr++){
        int base=0;for(int group=0;group<PC_VECTOR_GROUPS;group++){
          if(m->post_n[rd][p][fr][group]<(uint64_t)required_q[group]||
             m->post_q[rd][p][fr][group]!=required_q[group]||
             vector_counts[rd][p][fr][group]!=required_q[group])goto bad;
          int unique=0;
          for(int i=0;i<m->records;i++)if(m->row[i].round==rd&&m->row[i].ply_bin==p&&
               m->row[i].frontier==fr&&vector_post_group(m->row[i].allocation_slot)==group){
            int seen=0;for(int j=0;j<i;j++)if(m->row[j].round==rd&&m->row[j].ply_bin==p&&
               m->row[j].frontier==fr&&vector_post_group(m->row[j].allocation_slot)==group&&
               m->row[j].source_match_index==m->row[i].source_match_index){seen=1;break;}
            if(!seen)unique++;
          }
          if(unique<m->source_minimum)goto bad;
          base+=vector_counts[rd][p][fr][group];
                                ...
                                BoundReservoir *out,unsigned char file_hash[32])
{
    ...
    n=getline(&line,&cap,f);if(n<0)goto bad;
    if(!strcmp(line,"LCPOLICYCOST-TRAIN-RESERVOIR-V5\n"))out->is_vector=0;
    else if(!strcmp(line,"LCPOLICYCOST-VECTOR-RESERVOIR-V2\n"))out->is_vector=1;
    else goto bad;
    ...
    while((n=getline(&line,&cap,f))>=0){
        ...
        if(!out->is_vector){
          ...
          snprintf(cell,sizeof cell,"r%d:p%02d:f%d:g%d",a->round,a->ply_bin,
                   a->frontier,vector_post_group(a->allocation_slot));
                           ...
                           double *mean,double *se)
{
    ...
    *mean=*sum/n;
    /* Do not recover variance by subtracting two O(n * mean^2) terms.
     * V5 produced -nan for a constant 512-world terminal panel when that
     * cancellation landed seven ulps below zero.  Accumulating residuals is
     * stable, non-negative by construction, and leaves the sealed raw samples,
     * sum, sum-of-squares, and mean independently auditable. */
    double centered=0.0;
    for(int i=0;i<n;i++){double residual=x[i]-*mean;centered+=residual*residual;}
                         ...
                         double *action,double *draw)
{
    ...
    /* The complete-move probabilities are authoritative binary32 values.
     * Their exact double sum can exceed one by a few ulps even when the
     * underlying normalized policy is valid (v11's sole witness was
     * 1.0000000298023224).  Canonicalize only this representational fringe;
     * leave a material overshoot above eight binary32 epsilons untouched so
     * the frozen Python verifier still fails closed.  The conditional ratio
     * must retain the raw denominator, or the joint distribution would move.
     *
     * A legal semantic action can have exact zero binary32 mass after the
     * policy softmax underflows every complete move in that action.  Its
     * conditional draw distribution is irrelevant because it cannot enter a
     * positive-mass shortlist.  Encode canonical finite zero instead of 0/0. */
    double raw=a;
    if(a>1.0&&a<=1.0+0x1p-20)a=1.0;
    *action=a;*draw=raw>0.0?j/raw:0.0;
}
...
static int self_test(void)
{
    ...
    /* Exact regression for v5 run 33176041169, TRAIN shard 2 line 17.
     * The old sumsq-n*mean^2 formula yielded -1.6298145055770874e-9
     * and serialized an invalid JSON token, `-nan`. */
    double constant[512],sum=0.0,sumsq=0.0,mean=0.0,se=0.0;
    for(int i=0;i<512;i++)constant[i]=-50.800000000000111;
    sample_moments(constant,512,&sum,&sumsq,&mean,&se);
    if(!isfinite(se)||se<0.0||se>1e-12)return 10;
    /* Exact regression for v6 run 33213087155, TRAIN shard 88 line 2. */
    Move zero_moves[2]={{0}};float zero_prob[2]={0.0f,0.0f};double za,zd;
    zero_moves[0].card=40;zero_moves[0].draw=0;
    zero_moves[1].card=40;zero_moves[1].draw=1;
    policy_terms(zero_moves,zero_prob,2,0,&za,&zd);
    if(za!=0.0||zd!=0.0||!isfinite(zd))return 11;
    /* Exact regression for v11 run 33313785880, TRAIN allocation 1223.
     * These three authoritative binary32 probabilities sum to
     * 1.0000000298023224 in double.  The serialized core is canonical one,
     * while conditional probabilities retain that exact raw denominator. */
    Move fringe_moves[3]={{0}};
    union{uint32_t u;float f;} fringe_prob[3]={
        {.u=UINT32_C(0x3dae27a8)},
        {.u=UINT32_C(0x3ee3cd47)},
        {.u=UINT32_C(0x3ef0a8d0)}};
    for(int i=0;i<3;i++){fringe_moves[i].card=46;fringe_moves[i].draw=i==2?5:i;}
    float fringe_p[3]={fringe_prob[0].f,fringe_prob[1].f,fringe_prob[2].f};
    double fa,fd,raw=(double)fringe_p[0]+fringe_p[1]+fringe_p[2];
    policy_terms(fringe_moves,fringe_p,3,0,&fa,&fd);
    if(raw!=1.0000000298023224||fa!=1.0||
       fd!=(double)fringe_p[0]/raw||!isfinite(fd))return 12;
    float material_p[3]={0.4f,0.4f,0.4f};
    policy_terms(fringe_moves,material_p,3,0,&fa,&fd);
    if(!(fa>1.0+0x1p-20))return 13;
...
static void usage(FILE *f,const char *argv0)
{
    fprintf(f,
      ...
      "[--reservoir-per-cell N] [--smoke-seed 20280729...]\n"
      ...
      "seed must be in the burned 20280729 namespace and is labeled unusable "
      ...
      "smoke seed, every policy-cost-v2 fixed seed and 20261229 smoke seed, "
      "202612010101, all policy-cost-v3 fixed seeds in "
      "20270110/11/12/13/14/15/16/21/22, every 20270129 smoke seed, "
      "202701010101, all policy-cost-v4 fixed seeds in "
      "20270210/11/12/13/14/15/16/21/22, every 20270229 smoke seed, "
      "202702010101, all policy-cost-v5 fixed seeds in "
      "20270310/11/12/13/14/15/16/21/22, every 20270329 smoke seed, "
      "202703010101, all policy-cost-v6 fixed seeds in "
      "20270410/11/12/13/14/15/16/21/22, every 20270429 smoke seed, "
      "202704010101, all policy-cost-v7 fixed seeds in "
      "20270510/11/12/13/14/15/16/21/22, every 20270529 smoke seed, "
      "202705010101, 202804010101, all policy-cost-v17 fixed seeds in "
      "20280410/11/12/13/14/15/16/21/22, every 20280429 smoke seed, "
      "202805010101, all policy-cost-v18 fixed seeds in "
      "20280510/11/12/13/14/15/16/21/22, every 20280529 smoke seed, "
      "202806010101, all policy-cost-v19 fixed seeds in 20280610/11/12/13/14/15/16/21/22, every 20280629 feasibility-smoke seed, 202807010101, and every 20280729 "
      "feasibility-smoke seed are "
      "permanently outside all fixed "
