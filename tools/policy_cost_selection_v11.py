...
* ``SELECT`` compares exactly two frozen policy-floor configurations at the
  already-maintained all-ply search onset with source-match-clustered
...
SELECT_ALPHA = 0.05
SELECT_BOOTSTRAP_SEED = 202710150101
...
def _canonical_post_stratum(
    ...
    name = (
        ...
        f"f{int(frontier_present)}:g{min(allocation_slot, 2)}"
    )
    return (
...
def _discovery_manifest(
    ...
    expected_cell_count = 3 * (len(PLY_BOUNDARIES) - 1) * 2 * 3
    ...
    for round_index in range(3):
        for ply_stratum in range(len(PLY_BOUNDARIES) - 1):
            for frontier in (False, True):
                ...
                if slots is None or set(slots) != {0, 1, 2}:
                    raise InferenceError(
                        f"{stage} discovery base cell {key!r} is incomplete"
                    )
                for slot, expected_quota in enumerate((22, 21, 21)):
...
def _conservative_key(config_id: str) -> tuple[int, float, str]:
    ...
    # The maintained Objective-3 actor already searches at every ply, so
    # onset is fixed at zero.  min() conservatively chooses the higher (2%)
    # floor when performance is statistically tied.
